...
| --------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| [Add a custom invariant](./custom-invariant)  | Write a `verifyPlanRules` entry via a plugin to enforce team-specific rules in `arbiter check plan` |
| [Add a custom generator](./custom-generator)  | Extend arbiter core with a new language or archetype (contributor path)                             |
| [Write an arbiter configure plugin](./plugin) | Emit files and rules from a separate npm package — no arbiter fork needed                           |
