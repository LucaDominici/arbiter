---
title: 'Invariant Catalog — arbiter'
doc_version: '1.0.0'
status: active
last_review: '2026-09-25'
owner: ''
canonical_id: 'invariant-catalog'
tags: ['audience/dev', 'kind/governance']
related: ['docs/internal/SYSTEM/CANON.md', 'src/invariants/catalog.ts']
---

# Invariant Catalog

The complete invariant, enforcement, gate, debt, security, and process-canon reference for arbiter.
...
- **INV-90:** Evidence bundle schema compliance
  ...
  - `src/evidence/provenance.ts` (`buildProvenance`/`validateProvenance`/`formatProvenance`) is the shared TS module backing both this bundle path and the `.evidence/SUMMARY.json` path (`src/evidence/summary.ts`, `arbiter check evidence`) — the JSON Schema stays the enforced source of truth for the bundle gate itself.
...
- **INV-128:** Conformance script generated
  - _Enforcement:_ `scripts/conformance.mjs` — emitted for all governed targets by the always-on registry entry `conformance` in `src/generators/registry.ts` (`generateConformanceScript`, the sole emitter — deliberately not in `check-all.ts` UNCONDITIONAL_EMISSIONS, #1578) from `src/templates/scripts/conformance.mjs.ejs` (CANON-01/04/11). The standalone `conformance` command was retired in the T2 command-surface cut (`commands/conformance.ts` deleted; `src/conformance/{render,score,doc-probes}.ts` cut, while `gate-proofs.ts`/`dimensions.ts`/`engine.ts`/`shared.ts` survive for `doctor --prove-gates`), so the emitted runner is self-contained: it prints where the governance scorecard lives now — `npx @getarbiter/cli audit readiness` (level band + missing items) — and exits. Wired as advisory (`runWarnCheck`) in the generated `scripts/check-all.mjs` L2 behind an `existsSync` guard — informational only, does not hard-fail the gate. Generator `src/generators/conformance.ts` (skipIfExists:true). Exit codes (INV-53): 0=advisory OK, 2=ERROR. (#1398, #2628)
...
- **INV-130:** E2E flaky-test quarantine annotates but never suppresses, and cannot rot
  - _Enforcement:_ `<project>/scripts/check-e2e-quarantine.mjs` (L1, Track-B — not an arbiter self-gate) — emitted unconditionally for all governed targets via `src/generators/check-all.ts` UNCONDITIONAL_EMISSIONS from `src/templates/scripts/check-e2e-quarantine.mjs.ejs` (CANON-01/04/11), alongside the library `src/templates/scripts/lib/e2e-reliability.mjs.ejs` it imports (deterministic failure fingerprint, INFRA/FLAKE/REGRESSION classify, initial→single-test→spec retry ladder, R0–R4 risk tier that fail-closes to R4, append-only JSONL ledger, quarantine schema). Wired HARD (`runCheck`) at L1 in the generated `scripts/check-all.mjs`. A quarantine entry ANNOTATES a known-unstable test but never suppresses it — quarantined tests still run and report; the gate enforces the registry (`.arbiter/e2e/quarantine.json`) cannot rot into a permanent silent mute (every entry needs the full field set + a FUTURE `expires`). Self-SKIPs (exit 0) when no registry is present. exit 0=PASS/absent, 1=FAIL (expired/malformed), 2=ERROR per INV-53 (#1445).
    **#1817 (A3):** retries hide races, so `retryLadder({ tier: 'smoke' })` force-truncates the ladder to a single `['initial']` attempt — zero retries for `@smoke`, non-bypassable (overrides caller-supplied `opts.scopes`). Quarantine rot is also surfaced as a conformance dimension, `DISC-e2e-quarantine` (tier-1 must-pass), via `probeE2eQuarantine` in `src/conformance/dimensions.ts` / wired in `src/conformance/gate-proofs.ts` (consumed by `arbiter status health --prove-gates` — the `conformance` command that used to run this probe directly was removed in the T2 cut) — an arbiter self-gate (Track-A), distinct from the emitted Track-B script above. AC: a repo with an expired quarantine entry fails `arbiter status health --prove-gates`.
...
- **INV-140:** Every identifier scheme is registered, collision-free, and its citations resolve
  - _Enforcement:_ `scripts/check-id-registry.mjs` (L1, self) — validates the machine-parsed block in `docs/internal/SYSTEM/ID-REGISTRY.md` against `schemas/id-registry.schema.json` via the shared validator in `scripts/lib/agent-return-validate.mjs` (CANON-16, no second validator). Proves no two schemes can match one identifier (each anchored pattern is expanded into a canonical sample and cross-matched against every other regex — this is what closed the `MN` collision between product milestones and methodology measures), that every declared SSOT resolves on disk, and that every `OD-NN` cited anywhere in the tree resolves to a row in `docs/internal/SYSTEM/OD-REGISTRY.md` (previously cited in five places with no file defining it). A `staged` row is a dated obligation and fails once its `expires` passes, the INV-31 discipline applied to the ontology. Staged self-only (`scripts/canon01-self-only.json`, expiry 2026-12-31): the Track-B twin ships when governed targets carry their own registry. Verified by `__tests__/scripts/check-id-registry.test.ts`. exit 0=PASS, 1=violation, 2=ERROR per INV-53.

- **INV-141:** No artifact type exists as documentation alone — every active scheme is wired
  - _Enforcement:_ `scripts/check-ontology-wired.mjs` (L1, self) — the anti-prose meta-gate. For every `active` row in the ID registry it proves the gate script is registered on the side its `track` names (`scripts/check-all.mjs` for self, `src/templates/scripts/gate-registry.yml.ejs` for target, so a correctly-declined Track-B gate is never mistaken for an unwired one), the `tool` verb resolves to a `.command()` in `src/cli.ts`, and the `hook` file exists, is registered in `.claude/settings.json`, and **covers the row's SSOT** — existence and registration are necessary and were never sufficient, and four rows passed this check while naming a hook whose dispatch table matched none of their instances (#2480 wave 8). An unregistered hook never fires; an uncovering one never fires either. A **fourth leg** since wave 8: a declared `graphNode` must be a NodeKind the graph actually has. Six rows asserted one while `src/graph/model.ts` had none of them — the same shape as the hook column, a statement of intent reading as a statement of fact. Most schemes have no graph node and that is fine; the rule is only that a _declared_ one is real. `staged` rows are exempt (INV-140 owns their deadline) but counted against a monotone ratchet in `scripts/data/ontology-baseline.json`; there is deliberately no `--allow-increase`, so growth lands in the diff beside its justification. Staged self-only, downstream of INV-140. Verified by `__tests__/scripts/check-ontology-wired.test.ts`. exit 0=PASS, 1=violation, 2=ERROR per INV-53.

- **INV-142:** An edited ontology artifact is schema-valid at edit time, not at merge time
  - _Enforcement:_ `.claude/hooks/post-edit-artifact-schema.mjs` (`PostToolUse` → `Edit|Write`, self) — registered in `.claude/settings.json`. Carries a table of registered instances (path or directory prefix, schema, and how to extract the document: the whole file as JSON or YAML, or a fenced JSON block between sentinels) and validates whatever was just written, blocking with exit 2 — the only blocking code under the hook protocol, since exit 1 prints where the agent never sees it. One hook rather than one per artifact: each wave adds a single line and its type becomes edit-time enforced. The table is the mechanism and was also the gap: until wave 8 it held two entries while four ID-registry rows named this hook as their edit-time enforcement, so no milestone, epic, source record or use case was ever checked at the edit. INV-141 now resolves each row's SSOT through this table, so the claim cannot be made again without being true. Fails OPEN on its own infrastructure (validator or schema unloadable ⇒ exit 0), because blocking an unrelated edit when a dependency moved is worse than the failure it prevents; the CI gate is the backstop. Staged self-only. Verified by `__tests__/hooks/post-edit-artifact-schema.test.ts`.

- **INV-143:** The arbiter <-> forma schema contract is pinned and gated on both sides
  - _Enforcement:_ `scripts/check-forma-contract.mjs` (L1, self) — `schemas/CONTRACT.json` names each shared schema's owning repository, path, sha256 and vendoring consumers; both repos hold a byte-identical copy and gate their own half (forma runs the mirror `scripts/check-arbiter-contract.mjs` in `npm test` and as its own CI step). Editing a shared shape without re-pinning turns the owning repo red; a stale vendored copy turns the consuming repo red. With both checkouts side by side the gate also compares the two manifests byte-for-byte and re-hashes the sibling's live files; without a sibling that half SKIPS OUT LOUD, because a cross-repo check that silently does nothing is the failure the contract exists to prevent. Confirmed self-only: the subject is the boundary between these two repositories, which a governed target owns neither side of. Verified by `__tests__/scripts/check-forma-contract.test.ts`. exit 0=PASS, 1=violation, 2=ERROR per INV-53.

- **INV-144:** The architecture document is a filled structure, not a surviving skeleton
  - _Enforcement:_ `scripts/check-arc42-slots.mjs`. **Self:** L1, `runCheck`, unconditional, wired in `scripts/check-all.mjs` as `arc42 slots (INV-144)`. **Track B is deliberately weaker, and the asymmetry is declared rather than left to be found:** the row in `src/templates/scripts/gate-registry.yml.ejs` is L2, `runWarnCheck`, gated on `enableDebtGates` — a freshly generated arc42 is hollow by construction, and a gate that made `arbiter init` produce a red repo would only teach people to delete it. Parses the architecture document into `ARC-01`..`ARC-12` and requires every slot the skeleton for that project's tier column provides; a section deleted from the document is a structural gap, not a simplification. The required set is **read** from the skeleton (resolved from `src/templates/` in a dev checkout, `dist/templates/` in an installed package) rather than restated, so the gate cannot hold a second opinion. Hollow slots ride a ratchet in `scripts/data/arc42-baseline.json` — `stubs`, plus `skeletonGaps` **keyed per column** so the Canvas skeleton nobody audits still cannot lose a slot. A counter present but non-numeric is an ERROR, not a bootstrap. The first clean run **writes** the baseline it measured, so the ratchet actually arms in a governed project. The governed project receives a thin runner delegating to `arbiter audit docs --arc42`; the engine ships via `package.json` `files[]`, enforced by the **derived** engine set in `scripts/check-tarball-contents.mjs` (#2335/#2480 — the hand-maintained list of the same paths did not ratchet and let a fourth engine ship unshipped). Verified by `__tests__/scripts/check-arc42-slots.test.ts` (43 cases), which reads the real skeletons and a package-shaped `dist`-only layout, not fixtures alone. exit 0=PASS, 1=violation, 2=ERROR per INV-53.

- **INV-145:** Adversarial review closes only when nothing above low severity survives
  - _Enforcement:_ `scripts/check-refutation-verdicts.mjs`, wired on **both tracks** already as `refutation majority (E2 #1943)` (`runWarnCheck` in `scripts/check-all.mjs`; registry row in `src/templates/scripts/gate-registry.yml.ejs`) — this axis extends that gate rather than adding a second one over the same evidence (CANON-16). Marker-gated: with `.arbiter/evidence/agent-returns/<task>/refutation-required.json` present, every finding the skeptics **majority-UPHELD** at `critical`/`high`/`med` must appear in the marker's acted-on `findings`. The complement of the majority rule, not a duplicate: that one stops a phantom being acted on, this one stops the loop ending while something real is open. Severity is the **highest** any skeptic assigned. A finding below quorum or majority-REFUTED never blocks — one false alarm must not hold a wave hostage. `degraded: true` (no independent skeptic reachable) is reported on every run and never counts as an independent round. **Marker selection (#2860):** with a declared task (`.claude/.task/status.json` via `--repo-root`), the checker adjudicates the marker bound to that task, not `markers[0]`; with two or more markers present and no task identity it refuses to pick one (exit 2) rather than guess. **Quorum counting (#2860):** each skeptic **envelope** casts at most one vote per finding — an envelope repeating the same verdict does not inflate quorum, and a self-contradicting envelope (UPHELD and REFUTED on the same target) casts none. Verified by `__tests__/scripts/check-refutation-verdicts.test.ts`. exit 0=PASS, 1=violation, 2=ERROR per INV-53. CANON-24.

- **INV-146:** A milestone is done only when its exit criteria carry evidence
  - _Enforcement:_ `scripts/check-milestones.mjs`. **Both tracks:** self as `milestones (INV-146)` via `runCheck` in `scripts/check-all.mjs`, target via the `milestones` row in `gate-registry.yml.ejs`, emitted **with** `schemas/milestone.schema.json` (CANON-11). The emitted twin reads a fenced JSON block in `docs/MILESTONES.md` rather than YAML, and that is **forced**: `yaml` is arbiter's dependency, no emitted script imports it, and a Go or Python project receives 77 `.mjs` gates and **no** `package.json` in which to declare one — the same constraint that already shaped `docs/SOURCES.md` and `docs/USE_CASES.md`. Every rule is identical; reader, path and INV namespace (`AGENTS.md` on the target) are the three pinned divergences. Because the copies differ in their _reader_, the emitted one is proven by **execution**: the Track-B test renders it into a project-shaped tree and runs it — cycle, decay, fail-closed `done`, the epic join and both malformed-block paths (#2335). Validates `docs/internal/PRODUCT/MILESTONES.yml` against `schemas/milestone.schema.json`, then proves three things a document cannot hold: the dependency graph is **acyclic** (reported as the cycle path, not as a boolean), granularity **decays** with distance so a `later` milestone may not carry a `due` date, and `done`/`verified` is **fail-closed** on exit-criteria evidence — `verified` additionally requires each `evidence_ref` to resolve. A `github:` ref is accepted unresolved because this gate is offline by contract (INV-13). A missing `MILESTONES.yml` **SKIPs out loud** and is visible as `verdict: "skip"` under `--json`, so a project without a roadmap is distinguishable from one whose gate never ran. The decay rule is enforceable only since #2509 taught the shared validator `if`/`then`/`not`. The same SSOT carries the **epic index** (`EP-NN`, wave 8), read by the same gate: the milestone↔epic join is checked in **both directions**, an issue may be claimed once (as an epic or as a plain member, never both), a terminal epic status carries the same fail-closed `evidence_ref` rule, and a milestone cannot be `done` while an epic targeting it is open. The gate cannot compare a status against GitHub — offline by contract — and that gap is not hypothetical: all three epics first recorded here were already closed, two abandoned rather than delivered, while the file called them open. Verified by `__tests__/scripts/check-milestones.test.ts` (84 cases), tamper-proven in both directions on every rule. exit 0=PASS or SKIP, 1=violation, 2=ERROR per INV-53.

- **INV-147:** A cited source is quotable, and the quotation checks out
  - _Enforcement:_ `scripts/check-sources.mjs`. **Self:** L1, `runCheck`, wired in `scripts/check-all.mjs` as `sources tier 1 (INV-147)`. **Track B:** L1, from `src/templates/scripts/check-sources.mjs.ejs`. A URL in a document proves nothing — the page can change, or can never have said what it is cited for. A source is admitted only with a **committed, citation-length excerpt** plus the sha256 of that excerpt, so every quotation is checkable offline against evidence that cannot drift unnoticed. **Both halves are load-bearing:** a substring check alone passes on an excerpt edited after the fact; a hash alone says nothing about whether the quotation appears at all. Deliberately **not** done: the `url` is recorded provenance and is never dereferenced (a gate that fails when a website is down fails for a reason unrelated to the claim it guards), and **relevance is a judgement, not a decidable property** — tier 1 refuses to fake it, leaving that to independent skeptics (tier 2) and graph reachability (tier 3). Missing `SOURCES.md` **SKIPs out loud**. **Both tracks:** `src/templates/scripts/check-sources.mjs.ejs`, wired through the `sources` row in the emitted gate registry, with `schemas/source-record.schema.json` emitted beside it — shipping the rule without its contract would have handed a governed project an error the moment it recorded a source, which is why it stayed self-only until now. The one divergence is the registry path (`docs/SOURCES.md` in a target), pinned in `.dogfood-divergences.json`. The `SRC` row stays `staged` until its CLI verb and hook land with tiers 2 and 3. Verified by `__tests__/scripts/check-sources.test.ts` (12 cases), tamper-proven on arbiter's own two recorded sources in both directions. exit 0=PASS or SKIP, 1=violation, 2=ERROR per INV-53.

- **INV-148:** A runbook names the invariants it handles, and what it names is real
  - _Enforcement:_ `scripts/check-runbook-coverage.mjs` (L1, self) — wired in `scripts/check-all.mjs` as `runbook coverage (INV-148)`, ratchet in `scripts/data/runbook-baseline.json`. Two directions, **deliberately asymmetric** because only one is honestly pass/fail today. **Hard:** every doc whose frontmatter `tags` declare `kind/runbook` carries a unique `RB-NN` `canonical_id` and a non-empty `handles:` whose every `INV` id resolves in the catalog — a dangling ref reads as coverage while covering nothing, and a runbook handling nothing named is a document about a topic rather than a response to a failure. **Ratcheted:** operational-tier invariants no runbook handles — a _debt counter_, labelled one, because 49 operational invariants against two runbooks would make a hard rule red on arrival and baselined into meaninglessness. Discovery reads **frontmatter, never greps**: `docs/GOVERNANCE.md` and `docs/INDEX.md` both contain the literal string `kind/runbook` while merely listing tags, so a grep would have made the gate's first finding its own false positive. Self-only, declared rather than left to be found: the `RB` scheme is `staged` in the ID registry with a dated expiry and the Track-B emission lands with it. Unlike the ADR ratchet, `--update-baseline` **records** a rise rather than refusing it — the counter is over a catalog this gate does not own, and adding an operational invariant legitimately raises it (INV-08); what stays refused is a _silent_ rise. Verified by `__tests__/scripts/check-runbook-coverage.test.ts` (41 cases), tamper-proven in both directions on every rule. exit 0=PASS or SKIP, 1=violation, 2=ERROR per INV-53.

- **INV-149:** A use case names an actor, a goal, and features that exist
  - _Enforcement:_ `scripts/check-use-cases.mjs` — wired on **both** tracks: self as `use cases (INV-149)` via `runCheck` in `scripts/check-all.mjs`, target via the `use-cases` row in `src/templates/scripts/gate-registry.yml.ejs`, emitted **with** `schemas/use-case.schema.json` (CANON-11 — a rule shipped without its contract dies on `MODULE_NOT_FOUND` the moment a project uses it, and a missing schema is exit 2 here rather than a silent skip). A use-case matrix decays by **rename**: a feature is renamed, every use case pointing at the old id keeps reading like coverage, and nothing notices — so the central rule is that every `featureId` **resolves** into the feature matrix. The schema carries the rest: an actor, a goal in the actor's terms, and `featureIds` `minItems: 1`, because a use case demanding no feature is a promise with nothing behind it. The scenario **join** is checked both ways — a scenario naming a use case that does not exist fails, and a use case claiming `exercised` that no scenario walks fails, since status is not a walk. This gate does its real work on the **target** track, and the asymmetry is structural, not an omission: arbiter's 62 matrix rows are cross-cutting capability areas, so one of its use cases would name nearly all of them and the link would carry no information; in a product a use case names one or two features, and that ratio is what makes the edge worth checking. The self copy therefore **SKIPs out loud** (`verdict: "skip"` under `--json`), and the emitted twin is proven to **run** — `__tests__/templates/track-b-evidence-gates.test.ts` renders it into a project-shaped tree and executes it, every failure path included, because a gate whose rules are exercised nowhere in CI is a gate that has never run (#2335). exit 0=PASS or SKIP, 1=violation, 2=ERROR per INV-53.

- **INV-150:** architecture.deny edges must name declared components
  - _Enforcement:_ `validateArchitecture` in `src/config/schema.ts`, called from `validateConfig` on every `arbiter.json` load (#2834, ADR-123). The optional `architecture` section declares components (a name mapped to glob patterns) and deny edges as `"from -> to"` strings, where `to` may be `*` to forbid a component from importing any other declared component. A deny edge naming a component that was never declared is a silent no-op at best and a typo hiding a missing rule at worst — this invariant makes that fail loudly at config-load time instead of quietly doing nothing. **Scope, stated plainly:** this covers the declaration only. The generator (`src/generators/boundaries.ts`) renders declared components/deny into `eslint.config.boundaries.mjs` in place of the fixed hexagonal layers, and the emitted gate (rendered from `src/templates/boundaries/check-boundaries.mjs.ejs`) genuinely fails when a denied import exists — but the `ts-boundaries` entry in `src/templates/scripts/gate-registry.yml.ejs` stays `soft: true`, so nothing yet turns that failure into a blocked PR. Removing `soft: true` is deferred to a follow-up ADR. Verified by `__tests__/config/schema.test.ts` (accept/reject cases including the undeclared-component edges) and `__tests__/generators/architecture-boundaries-2834.test.ts` (deny→allow conversion, byte-identical output when the section is absent).
