...
- **One-shot file** (`.arbiter/ssot-bypass`): write **the path the bypass authorizes on line 1 and the reason on lines 2+**, then retry the guarded edit. Since #2493 the marker is path-scoped: it is honoured only for that exact path, and only for the marker sitting in the worktree that path belongs to — a marker written for `X` can no longer be spent authorizing an edit to `Y`, or an edit in a different worktree. It is consumed (deleted) by an attempt on its own target, and left in place by an attempt on any other path so a pending authorization cannot be destroyed by an unrelated edit; an empty/blank reason is still consumed but does not bypass, and a marker naming no target authorizes nothing. Use for a single, explicit, reviewable edit (an ADR, or an owner directive naming the exact file) rather than a standing permission.
