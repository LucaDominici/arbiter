#!/usr/bin/env node
// arbiter — workflow top-level permissions gate (INV-76)
...
// FAIL-OPEN-INTENT: gate script; errors propagate via process.exit(1) at call sites
// CATALOG: checks that all .github/workflows/*.yml declare top-level permissions blocks.
// CATALOG: rejected fold-in into check-action-pins.mjs (different concern: perms vs pin integrity).
// CATALOG: rejected fold-in into check-ci-tiers.mjs (different concern: perms vs tier structure).
...
for (const file of files) {
  ...
  if (inline === 'write-all') {
    violations.push({
      ...
      reason: "top-level 'write-all' is forbidden",
    })
  }
}
