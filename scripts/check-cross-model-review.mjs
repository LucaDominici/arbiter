...
function writeFileContained(rootDir, relativePath, data, label) {
  ...
  try {
    ...
    error(
      `cannot write ${label} ${join(rootDir, relativePath)}: ${cause instanceof Error ? cause.message : String(cause)}`,
    )
    throw cause
  } finally {
...
if (configuredCrossModel === undefined) {
  if (envOverride !== true) {
    ...
    process.stdout.write(
      '[SKIP] [check-cross-model-review] skipped: crossModelReview not enabled\n',
    )
    process.exit(0)
  }
...
const changedPaths = [
  ...
  gitValue(['diff', '--cached', '--name-only'], 'index changes').split(/\r?\n/).filter(Boolean),
...
for (const [index, fulfilled] of artifact.fulfilled.entries()) {
  ...
  if (
    ...
    fail(
      `fulfilled[${index}].envelope must be under the active task's agent-return directory: ${envelope}`,
    )
  }
