...
// arbiter — PR size gate drift checker (INV-89)
...
if (existsSync(CONFIG_PATH)) {
  ...
  if (warn !== null && warn > MAX_WARNING_LINES) {
    process.stderr.write(
      `[FAIL] ${CONFIG_PATH}: warnLines=${warn} exceeds maximum ${MAX_WARNING_LINES} (INV-89)\n`,
    )
    violations++
  }
  if (error !== null && error > MAX_ERROR_LINES) {
    process.stderr.write(
      `[FAIL] ${CONFIG_PATH}: errorLines=${error} exceeds maximum ${MAX_ERROR_LINES} (INV-89)\n`,
    )
    violations++
  }
  ...
  if (violations > 0) {
    process.stderr.write(
      `check-pr-size-gate: FAIL — PR size gate configuration out of bounds (INV-89)\n`,
    )
    process.exit(1)
  }
  process.stdout.write(`check-pr-size-gate: OK — PR size gate configuration valid (INV-89)\n`)
} else {
  // No config file — check if size checking is referenced in any workflow
  if (existsSync(WORKFLOW_DIR)) {
    ...
    if (found) {
      process.stdout.write(
        `check-pr-size-gate: OK — PR size gate referenced in workflows (INV-89)\n`,
      )
    } else {
      process.stdout.write(
        `check-pr-size-gate: SKIP — no PR size gate configuration found (optional) (INV-89)\n`,
      )
