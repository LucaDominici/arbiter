...
function checkExpiry(expiresAt, label, file) {
  ...
  if (diffMs < 0) {
    process.stderr.write(
      `[EXPIRED] ${file}: ${label} — expiresAt: ${expiresAt}\n`,
    );
    failed++;
  } else if (diffDays <= WARN_DAYS) {
    process.stderr.write(
      `[WARN] ${file}: ${label} expires in ${diffDays} day(s) (${expiresAt})\n`,
    );
    warnings++;
  }
}
...
function checkJsonFile(filePath) {
  ...
  try {
    ...
    process.stderr.write(
      `[FAIL] ${filePath}: could not parse JSON — ${e.message}\n`,
    );
    failed++;
    return;
  }
  if (!Array.isArray(entries)) {
    process.stderr.write(
      `[FAIL] ${filePath}: expected a JSON array, got ${typeof entries}\n`,
    );
    failed++;
    return;
  }
...
function checkXmlFile(filePath) {
  ...
  for (let i = 0; i < tokens.length; i++) {
    if (tokens[i].type === "suppress") {
      const candidate =
        i > 0 && tokens[i - 1].type === "comment" ? tokens[i - 1] : null;
