...
const commandsDir = commandsArg
  ? resolve(commandsArg.split("=")[1])
  : resolve(root, "src/commands");
const testsDir = testsArg
  ? resolve(testsArg.split("=")[1])
  : resolve(root, "__tests__/commands");
...
if (commands.length === 0) {
  console.log(
    `[check-command-tests] FATAL: no *.ts files found in ${commandsDir} — wrong path?`,
  );
  process.exit(1);
}
...
if (violations > 0) {
  console.log(
    `[check-command-tests] FAIL: ${violations} command(s) lack test files`,
  );
  process.exit(1);
}
console.log(
  `[check-command-tests] OK — all ${commands.length} commands have test files`,
);
