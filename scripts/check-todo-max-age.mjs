 ...
 * "looked at nothing" (CANON-24; #2526).
 */
...
export function main(exitFn = process.exit) {
  ...
  for (const dir of scanDirs) {
    // #2526 (mirrors #2512's fix for the sibling check-no-orphan-todo.mjs gate): resolve(), not
    // join(), against baseDir. join('/repo', dir) does NOT reset on an absolute `dir` —
    // join('/repo', '/tmp/fixture/src') silently becomes '/repo/tmp/fixture/src', a path that
    // (almost certainly) does not exist, so the gate scanned nothing and exited 0: a green that
    // means "I looked nowhere", not "there is nothing to find". resolve() has POSIX/Node's
    // documented right-to-left semantics: an absolute segment discards everything to its left, so
    // an absolute `dir` is used as-is and a relative one is still joined under baseDir exactly as
    // before.
