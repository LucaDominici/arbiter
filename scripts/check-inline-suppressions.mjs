...
function isInsideStringLiteral(line, idx) {
  ...
  // on its own advisory text). Not a lexer: a quote inside a /regex/ literal is not
  // recognized, and a multi-line template literal opened on an earlier line is
  // invisible (real suppressions are single-line `//` comments, so this only risks
  // false positives on contrived multi-line strings, never false negatives). Once a
  // real (non-string) `//` comment start is seen before idx, the rest of the line is
  // a comment and any quotes in it (e.g. an apostrophe) are ignored, so idx is never
  // reported as "inside a string" merely because of a quote in a trailing comment.
  let i = 0
  let inStr = false
  let quote = ''
  while (i < idx) {
