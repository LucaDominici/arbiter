...
const testsDir = testsArg
  ...
  : resolve(root, "__tests__/generators");
...
if (violations > 0) {
  console.log(
    `[check-generator-tests] FAIL: ${violations} generator(s) lack unit tests`,
  );
  process.exit(1);
}
console.log(
  `[check-generator-tests] OK — all ${generators.length} generators have test files`,
);
