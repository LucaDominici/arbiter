...
if (!existsSync(manifestPath)) {
  process.stdout.write(
    `[hardness-drift] manifest not found: ${manifestPath}\n`,
  );
  process.exit(1);
}
...
const hookFiles = readdirSync(hooksDir)
  .filter(
    (f) =>
      (f.endsWith(".mjs") || f.endsWith(".mjs.ejs")) && f !== "lib.mjs.ejs",
  )
  .sort();
...
for (const entry of manifest.hooks) {
  ...
  if (!existsSync(fullPath)) {
    fail(
      `manifest entry '${entry.file}' points to non-existent file — drift detected`,
    );
  }
}
...
for (const entry of hardSpawnable) {
  ...
  if (!fixture) {
    fail(
      `${entry.file} is HARD+spawnable but has no fixture defined in manifest`,
    );
    continue;
  }
