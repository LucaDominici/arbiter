...
// #2675 Codex round-3: `--max-days --dir <valid>` parses '--dir' as the number, yielding NaN —
// every `diffDays > MAX_DAYS` comparison is then false, so no entry can ever violate the window.
// A malformed numeric flag must fail loud, never silently disable the check it configures.
if (!Number.isInteger(MAX_DAYS) || MAX_DAYS <= 0) {
  process.stderr.write(
    `check-suppression-expiry: --max-days ${JSON.stringify(MAX_DAYS_RAW)} must be a positive integer\n`,
  )
  process.exit(2)
}
