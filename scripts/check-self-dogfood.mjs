...
export function buildRenderContext(cfg = {}) {
  ...
  return {
    ...
    formatCommand:
      cfg.formatCommand ?? getNpmScript("format", "npx prettier --write ."),
    tools,
    governanceLevel,
    ...
    enableSecurityScanning:
      cfg.enableSecurityScanning ?? features.securityScanning ?? true,
    ...
    invariantTiers: cfg.invariantTiers ?? [
      "architectural",
      "governance",
      "data",
      "operational",
...
export function templateToMaterialized(templatePath) {
  ...
  if (idx === -1) {
    throw new Error(
      `Template path does not contain '${marker}': ${templatePath}`,
    );
  }
...
export function isConfigGated(templatePath, ctx) {
  if (
    templatePath.endsWith("hooks/guard-done-evidence.mjs.ejs") &&
    !ctx.enableEvidenceHarness
  ) {
    return true;
  }
  return false;
...
async function main() {
  ...
  const arbiterConfig = JSON.parse(
    readFileSync(join(repoRoot, "arbiter.json"), "utf-8"),
  );
  ...
  console.log(
    `[dogfood] ${skipped} template(s) skipped (config-gated or diverged).`,
  );

  if (drifted.length > 0) {
    console.error(
      `[dogfood] FAIL — ${drifted.length} template(s) have unexpected drift:`,
    );
    for (const d of drifted) {
      ...
      if (d.reason) {
        ...
        if (d.removed && d.removed.length > 0) {
          console.error(
            `  removed lines (in rendered but not in materialized):`,
          );
