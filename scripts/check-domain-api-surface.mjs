...
function main() {
  ...
  for (const resource of manifest.resources) {
    if (
      ...
      operational('Each resource needs a non-empty resource name and domainFields array.')
    }
    for (const field of resource.domainFields) {
      if (
        ...
        operational(`Invalid field in resource ${resource.resource}.`)
      }
    }
  }
  if (!helperPath) {
    ...
    for (const resource of manifest.resources) {
      for (const field of resource.domainFields) {
        if (field.persisted === true && !field.inRequestSchema && !field.inResponseSchema) {
          gaps.push({ resource: resource.resource, field: field.name })
        }
      }
    }
    if (gaps.length > 0) {
    ...
    const checked = manifest.resources.reduce((sum, resource) => {
      return sum + resource.domainFields.length
    }, 0)
    console.log(
