...
function collectComplexityViolations(cwd, metrics, collectionErrors) {
  ...
  if (collectionFailed) {
    const reason = `eslint complexity collection failed (exit ${result.status ?? 'none'}${result.signal ? `, signal ${result.signal}` : ''})`
  ...
  try {
    ...
    metrics.complexityViolations = {
      value: report
        ...
        .filter((message) => message.ruleId === 'complexity').length,
      unit: 'count',
      direction: 'lower-is-better',
    }
  } catch (err) {
