...
// `jscpd-linux-x64-gnu` package, but that binary is built against GLIBC ≥ 2.34;
...
// musl x64 platform package is a *static-pie* ELF with no libc dependency, so
...
// jscpd renamed its platform packages once already (`cpd-linux-x64-musl` ->
// `jscpd-linux-x64-musl` in 5.0.16, #2370), so the package name is derived from
// jscpd's own optionalDependencies rather than hardcoded.

/**
 * The optionalDependencies key jscpd declares for the musl x64 Linux platform
 * package. Derived instead of hardcoded because jscpd has already renamed this
 * package once (#2370).
 * @param {{optionalDependencies?: Record<string, string>}} jscpdPkgJson - parsed jscpd package.json
 * @returns {string | null}
 */
export function deriveStaticMuslPkgName(jscpdPkgJson) {
  const optionalDeps = jscpdPkgJson?.optionalDependencies ?? {}
  return Object.keys(optionalDeps).find((name) => name.endsWith('-linux-x64-musl')) ?? null
}
...
function resolveStaticCpd(jscpdPkgDir) {
  ...
  let pkgJson
  try {
    pkgJson = JSON.parse(readFileSync(join(jscpdPkgDir, 'package.json'), 'utf-8'))
  } catch {
    return null
  }
  const version = pkgJson.version
  const staticPkgName = deriveStaticMuslPkgName(pkgJson)
  if (!version || !staticPkgName) return null
  ...
  try {
    ...
    const pack = run('npm', ['pack', `${staticPkgName}@${version}`, '--silent'], { cwd: tmp })
    if ((pack.status ?? 1) !== 0) return null
    const tgz = readdirSync(tmp).find((f) => f.endsWith('.tgz'))
    if (!tgz) return null
    const untar = run('tar', ['xzf', tgz, '-C', tmp], { cwd: tmp })
    if ((untar.status ?? 1) !== 0) return null
    // jscpd 5.0.16 ships the binary at package/bin/jscpd (was
    // package/cpd-bin/cpd pre-rename, #2370). Try both so a consumer project
    // pinned to an older jscpd (whose static package still uses the old
    // layout) still resolves.
    const extracted = [
      join(tmp, 'package', 'bin', 'jscpd'),
      join(tmp, 'package', 'cpd-bin', 'cpd'),
    ].find((p) => existsSync(p))
...
export function countPublicApi(cwd) {
  let count = 0
  for (const rel of walkRepo(cwd)) {
    if (!rel.startsWith('src/') || !/\.m?ts$/.test(rel)) continue
    count += readFileSync(resolve(cwd, rel), 'utf-8')
      .split('\n')
...
function readReusedCoverageSummary(cwd, opts) {
  const rawPath = opts.coverageSummaryPath
  if (typeof rawPath !== 'string' || rawPath.length === 0) {
    throw new Error('coverage summary path is missing')
  }
  const summaryPath = resolve(cwd, rawPath)
  if (isAbsolute(rawPath) || relative(cwd, summaryPath).startsWith('..')) {
    throw new Error('coverage summary path must stay inside the project root')
  }
  const stat = statSync(summaryPath)
  if (!stat.isFile()) throw new Error(`coverage summary is not a file: ${summaryPath}`)
  if (opts.coverageStartedAt !== undefined) {
    if (!Number.isFinite(opts.coverageStartedAt)) {
      throw new Error('coverage summary freshness timestamp is invalid')
    }
    if (stat.mtimeMs < opts.coverageStartedAt) {
      throw new Error(`coverage summary is stale: ${summaryPath}`)
    }
  }
 ...
 * @param {{spawnCoverage?: (cwd: string, args: string[]) => {status: number|null, stdout: string, stderr: string} | null, coverageSummaryPath?: string, coverageStartedAt?: number}} [opts]
...
export function collectMetrics(cwd, collectionErrors = [], opts = {}) {
  ...
  if (opts.coverageSummaryPath !== undefined) {
    try {
      Object.assign(metrics, coverageMetricsFromSummary(readReusedCoverageSummary(cwd, opts)))
    } catch (err) {
      const reason = `gate coverage summary unreadable: ${err instanceof Error ? err.message : String(err)}`
    ...
    if (coverageRaw !== null) {
      try {
        ...
        Object.assign(metrics, coverageMetricsFromSummary(summary))
      } catch (err) {
