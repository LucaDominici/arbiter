...
export function collectMetrics(cwd) {
  ...
  try {
    const summary = JSON.parse(
      readFileSync(
        resolve(cwd, ".coverage-tmp/coverage-summary.json"),
        "utf-8",
      ),
    );
  ...
  if (eslintRaw !== null) {
    try {
      ...
      metrics.eslintErrors = {
        value: report.flatMap((f) => f.messages).filter((m) => m.severity === 2)
          .length,
        unit: "count",
    ...
    try {
      ...
      metrics.complexityViolations = {
        value: report
          ...
          .filter((m) => m.ruleId === "complexity").length,
        unit: "count",
        direction: "lower-is-better",
      };
    } catch {
