...
//
// WHY THIS IS TRACK-B-SHAPED, stated rather than discovered later: arbiter itself has no use
// cases, and the reason is structural rather than an omission. Its 62 feature-matrix rows are
// cross-cutting CAPABILITY AREAS (architecture enforcement, static analysis, CI/CD wiring), so a
// use case here would name nearly all of them and the link would carry no information. Measured
// against a real product's matrix, a use case names ONE or TWO features — that ratio is what makes
// the edge worth checking. So this gate SKIPS on arbiter's own track and does its work in a
...
const SSOT_REL = join('docs', 'internal', 'PRODUCT', 'USE_CASES.md')
const MATRIX_REL = join('docs', 'internal', 'PRODUCT', 'FEATURE_MATRIX.md')
