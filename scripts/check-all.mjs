...
if (isMain) {
  ...
  const vitestOptions = vitestEnv ? { env: vitestEnv } : {}
  ...
  function runChecks() {
    ...
    runWarnCheck('orchestrator coverage (#1410)', 'node', [
    ...
    runCheck('orchestration integrity (#2387)', 'node', [
      'scripts/check-orchestration-integrity.mjs',
    ])
    ...
    runCheck('canon enforcement parity (B1)', 'node', [
      'scripts/check-canon-enforcement-parity.mjs',
    ])
    ...
    runCheck('anti-drift: suppression rationale', 'node', [
    ...
    runCheck('anti-drift: workflow integrity', 'node', [
    ...
    const selfConfig = JSON.parse(readFileSync(resolve('arbiter.json'), 'utf8'))
    ...
    if (subcommand === 'check') {
      runCheck('unit tests', 'npm', ['test'], vitestOptions)
    }
    runCheck(
      'greenfield smoke',
      'npx',
    ...
    if (subcommand !== 'check' && !preflight) {
      ...
      // who installs @getarbiter/cli as a dependency. This packs the publishable tarball,
      ...
      runCheck('emission coherence (INV-123)', 'node', [
      ...
      runWarnCheck('audit dry-pass (E3 #1943)', 'node', [
