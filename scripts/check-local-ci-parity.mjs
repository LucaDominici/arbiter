...
try {
  ...
  process.stderr.write(
    `check-local-ci-parity: cannot parse ${LOCAL_RESULT_PATH}: ${err.message}\n`,
  );
  process.exit(2);
}
...
try {
  ...
  process.stderr.write(
    `check-local-ci-parity: cannot parse CI gate result: ${err.message}\n`,
  );
  process.exit(2);
}
...
if (!localResult.parityContentHash || !ciResult.parityContentHash) {
  skip(
    "gate result missing parityContentHash — regenerate with check-all.mjs L2",
  );
}
...
process.stderr.write(
  "check-local-ci-parity: FAIL — local↔CI parity drift detected\n\n",
);
process.stderr.write(`  local:  ${localResult.parityContentHash}\n`);
process.stderr.write(`  ci:     ${ciResult.parityContentHash}\n\n`);

const localMap = new Map(
  (localResult.parityGates ?? []).map((g) => [g.name, g.pass]),
);
const ciMap = new Map(
  (ciResult.parityGates ?? []).map((g) => [g.name, g.pass]),
);
