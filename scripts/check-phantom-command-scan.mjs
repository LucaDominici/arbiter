...
// cites `arbiter configure plugin add` inside a fenced block and nothing ever looked at it.
...
//
// #2408 also closes the root-file hole: PRIVACY.md was the ONLY root-level file in
// this list, so `arbiter configure plugin add <name>` sat in CONTRIBUTING.md — in plain inline
// backticks, the shape this gate was already built to catch — and no run ever opened
// the file. The set now mirrors the root files check-doc-links/check-doc-style
// already scan, so "is it a hand-authored root doc?" has ONE answer across the gates.
const ROOT_FILES = [
  'PRIVACY.md',
  'AGENTS.md',
  'README.md',
...
const ROOTS = argValue('roots')
  ...
  : [...ROOT_FILES, 'docs', 'website', '.claude', 'src/templates', 'examples']
...
// an audit report's job is to quote OTHER files' broken citations as EVIDENCE (`arbiter lifecycle checkpoint`,
...
// `arbiter configure plugin add` sit on the public website and in an example README, and
...
/** Record one fenced shell line's `arbiter <cmd> [sub]` invocation, if it has one. */
function recordFencedCommand(line, commands, pairs) {
  const m = line.match(FENCED_COMMAND_RE)
  if (!m || PROSE_STOPWORDS.has(m[1])) return
  commands.add(m[1])
...
export function extractFencedCitations(markdown) {
  ...
  for (const raw of markdown.split('\n')) {
    ...
    recordFencedCommand(line, commands, pairs)
 ...
 *
 * - aliases resolve to their canonical command first (`git worktree list` is
 *   `worktree list`; `arbiter check tdd` is `validate tdd` — cli.ts:1172);
