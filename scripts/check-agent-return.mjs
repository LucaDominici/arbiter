...
// CATALOG: envelope file under .arbiter/evidence/agent-returns/** against schemas/agent-return.schema.json,
...
// CATALOG: (.arbiter/agents-dispatched/<task>.json) so dispatched agents whose returns evaporated into
...
import { isForeignSidecar, locateDispatchSidecar } from './lib/evidence-binding.mjs'
...
const NON_ENVELOPE_MARKER = 'refutation-required.json'
...
// #2912 — one dispatch sidecar per task; `--sidecar=` still names the file explicitly.
const SIDECAR_AT = arg('sidecar', argv)
  ? { path: resolve(arg('sidecar', argv)), legacy: false }
  : locateDispatchSidecar(repoDefault, activeTaskId())
const SIDECAR = 'path' in SIDECAR_AT ? SIDECAR_AT.path : ''
...
function main() {
  ...
  try {
    const walk = (dir) => {
      for (const entry of readdirSync(dir)) {
        ...
        else if (entry.endsWith('.json') && entry !== NON_ENVELOPE_MARKER) envelopeFiles.push(full)
      }
    }
    walk(EVIDENCE_DIR)
  } catch (err) {
  ...
  if (ENFORCE && 'error' in SIDECAR_AT) {
    process.stderr.write(`[check-agent-return] ERROR: ${SIDECAR_AT.error}\n`)
    return 2
  }
  if (ENFORCE && existsSync(SIDECAR)) {
    try {
      const sidecar = JSON.parse(readFileSync(SIDECAR, 'utf-8'))
      // #2399: the legacy sidecar is shared by every branch — one recorded for another
      ...
      const foreign = isForeignSidecar(sidecar, activeTaskId(), SIDECAR_AT.legacy)
      if (foreign) {
        process.stdout.write(
