...
customization *is* that file's function.
...
- `arbiter diff` now reports a customized `scripts/check-all.mjs` as *withheld* rather than
  *changed* — which is what `update` actually does with it.
...
Upgrade note: if you *want* the template's gate entrypoint, run
