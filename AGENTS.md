...
- **INV-128:** Conformance script generated
  - _Enforcement:_ `scripts/conformance.mjs` — emitted for all governed targets by the always-on registry entry `conformance` in `src/generators/registry.ts` (`generateConformanceScript`, the sole emitter — deliberately not in `check-all.ts` UNCONDITIONAL_EMISSIONS, #1578) from `src/templates/scripts/conformance.mjs.ejs` (CANON-01/04/11). The standalone `conformance` command was retired in the T2 command-surface cut (`commands/conformance.ts` deleted; `src/conformance/{render,score,doc-probes}.ts` cut, while `gate-proofs.ts`/`dimensions.ts`/`engine.ts`/`shared.ts` survive for `doctor --prove-gates`), so the emitted runner is self-contained: it prints where the governance scorecard lives now — `npx @arbiter/cli gold-audit` (level band + missing items) — and exits. Wired as advisory (`runWarnCheck`) in the generated `scripts/check-all.mjs` L2 behind an `existsSync` guard — informational only, does not hard-fail the gate. Generator `src/generators/conformance.ts` (skipIfExists:true). Exit codes (INV-53): 0=advisory OK, 2=ERROR. (#1398, #2628)
