...
- **INV-130:** E2E flaky-test quarantine annotates but never suppresses, and cannot rot
  - _Enforcement:_ `scripts/check-e2e-quarantine.mjs` (L1, Track-B — not an arbiter self-gate) — emitted unconditionally for all governed targets via `src/generators/check-all.ts` UNCONDITIONAL_EMISSIONS from `src/templates/scripts/check-e2e-quarantine.mjs.ejs` (CANON-01/04/11), alongside the library `src/templates/scripts/lib/e2e-reliability.mjs.ejs` it imports (deterministic failure fingerprint, INFRA/FLAKE/REGRESSION classify, initial→single-test→spec retry ladder, R0–R4 risk tier that fail-closes to R4, append-only JSONL ledger, quarantine schema). Wired HARD (`runCheck`) at L1 in the generated `scripts/check-all.mjs`. A quarantine entry ANNOTATES a known-unstable test but never suppresses it — quarantined tests still run and report; the gate enforces the registry (`.arbiter/e2e/quarantine.json`) cannot rot into a permanent silent mute (every entry needs the full field set + a FUTURE `expires`). Self-SKIPs (exit 0) when no registry is present. exit 0=PASS/absent, 1=FAIL (expired/malformed), 2=ERROR per INV-53 (#1445).
    ...
    **#1817 (A4):** the ~10-rule installable E2E determinism standard lives in `docs/GOVERNANCE/E2E_CONSTITUTION.md`, generated by `generateE2eConstitution` for any project with a Playwright harness (frontend-spa or backend-web-db).
...
- **INV-131:** TDD red→green evidence is re-verified on a fresh checkout in CI
  ...
  - _Evidence is owed per CHANGE, not per commit (#2217):_ task ids in a commit SUBJECT are verified individually, as before. A branch with **no** subject-scoped id that changes `src/` must carry **at least one** verified evidence among the tasks its commit BODIES cite (`Refs #NNN`) — the convention for a commit with no TDD cycle of its own, which used to parse to zero ids and pass **vacuously**. That evidence must have been produced on the branch (inherited evidence from `main` proves nothing). No commit type is trusted to declare itself exempt; a docs- or chore-only branch stays vacuous because it changes no source. `src/templates/**` counts as source. The failure names the two ways forward: record real evidence for a cited task, or move the source change onto its own branch whose subject carries the id.
  - _The pin is rebase-stable (#2116):_ `test_commit_sha` must be REACHABLE from HEAD, not merely present as an object — a pre-rebase commit lingering behind a stale branch is not history anyone can reach. Because a rebase rewrites every sha, evidence also records `test_blob_sha`, the RED test's content, which a rebase preserves: when the pinned commit is unreachable the RED commit is re-resolved from that blob and every downstream check runs against the resolved commit. Evidence recorded before the pin existed cannot be healed and fails loudly — re-record it.
