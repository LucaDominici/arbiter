...
- AC-2913.1: The hook only guards paths inside the repository; edits outside it are not refused.
- AC-2913.2: A plan that Ship's admission accepts is never refused by the hook for a missing key that admission does not require. Either both require `context:`, or neither does.
- AC-2913.3: The plan-anchor refusals that remain (for example, a red/green/refactor edit with no plan pointer) keep naming their exact remedy command. With AC-2 resolved as "neither requires `context:`", the missing-key refusal no longer exists.
