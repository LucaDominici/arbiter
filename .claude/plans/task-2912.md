...
- AC-1: After `lifecycle start` on a fresh task, `git status --porcelain` is empty: the shared dispatch sidecar committed by an earlier task is neither deleted nor rewritten.
- AC-2: The review-completion check still refuses a sidecar that belongs to another task. It judges by the task field it already reads, not by the file's absence.
- AC-3: A regression test runs `lifecycle start` in a repo whose HEAD tracks another task's sidecar, and asserts a clean tree.
...
leaves another task's committed legacy sidecar byte-identical with a clean tree (AC-1, AC-3), (c) reader
refuses a foreign legacy file and accepts its own per-task file (AC-2). GREEN adds (d) legacy read on exact
