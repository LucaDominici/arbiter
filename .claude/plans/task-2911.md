...
- AC-2911.1: The recorder's stale-binding refusal names the one check that failed (task, branch, sha, or the specific binding field), and gives `lifecycle preflight` as the remedy when the binding is the cause.
- AC-2911.2: When the ac-fit artifact is missing because the reviewer returned NOT-TESTED, `check-acceptance` lists those criterion ids and gives both remedies: the `[exact-main]` marker for a CI-only criterion, or an independent verifier envelope.
- AC-2911.3: The stale-derived-gates refusal names which derived input changed (plan bytes, manifest, or a contract authority file).
- AC-2911.4: Runtime and emitted twins stay identical, verified by the existing self-dogfood and bake checks. One test per refusal asserts its signal and remedy text.
