...
1. **Measure** — `npx @getarbiter/cli gold-audit --json`; read the band + `gaps[]` (no AI re-scoring).
...
4. **Re-audit behind the guards (fail-closed):** `npx @getarbiter/cli gold-audit --check`
