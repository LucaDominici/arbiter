...
if [ -n "$dispatch_file" ] && [ -f "$dispatch_file" ] && node -e 'const f=require("node:fs");try{const c=JSON.parse(f.readFileSync("arbiter.json","utf8"));process.exit(c.crossModelReview?.enabled===true?0:1)}catch{process.exit(1)}' && node scripts/check-cross-model-review.mjs >/dev/null 2>&1; then
...
Capture the done-evidence (runs the gate + pins source SHAs — releases the
`guard-done-evidence` hook):
