...
- **Acceptance criteria (merged)** — the frozen `AC-N` anchor VERBATIM, extended with security ACs and edge cases; extend the existing `## Acceptance Criteria` anchor, never a rival heading.
...
Write it in one pass — draft, then revise once before finalizing: the revise pass was part of the measured treatment, not optional polish.
