...
| `red-team-review` | `trunk-solo`: no pre-code dispatch; proceed to TDD and keep the independent final review. Other modes: dispatch tier-N red-team agents and record `.arbiter/evidence/redteam/<task-id>.json`; route CRITICAL → `red-team-rework` | 0 (trunk-solo); tier-N otherwise |
...
clean-HEAD `node scripts/check-all.mjs L3`; done-evidence and pre-push reuse that receipt while the
...
In `trunk-solo`, do not dispatch a pre-implementation panel or fabricate its evidence. The root
implementer owns the plan and threat model, then proceeds to TDD; the independent final diff review,
AC-fit, full gate and CI remain mandatory. In `peer-review` and `gated-review`, dispatch **N parallel
agents** (XS=1, S=2, Standard=3). Each agent self-selects an attack angle from: `security`,
...
2. Advance to red-team-review (no units yet — `checkHandoffGate` is not triggered here):
   ...
   arbiter ship #NNN --advance
   ```
3. In trunk-solo, advance directly to **red**. In other modes, advance after red-team-review
   completes. Pass the unit count from the plan (§7):
   ```bash
