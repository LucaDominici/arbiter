...
Every FEATURE_MATRIX capability must appear exactly once. A `VERIFIED` row's `proof` must be its
`.arbiter/evidence/rtm/<REQ-NNN>.json` envelope with verdict `PROVEN` and `subject_sha` equal to the
report's frozen subject; prose and authored transcripts are not proof.
