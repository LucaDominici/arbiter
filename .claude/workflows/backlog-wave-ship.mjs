...
3. Run \`arbiter ship ${it.issue}\` and follow the emitted result-first loop faithfully:
   mechanical plan admission → TDD red→green→refactor (test FIRST, lifecycle record-red) → freeze →
   the persisted treatment's independent final review + per-AC acceptance fit → one exact-subject full gate.
...
- TDD evidence: \`arbiter lifecycle record-red\` stamps the pre-commit BASE sha (no test there). After committing, set .arbiter/evidence/tdd/${it.issue}.json test_commit_sha to the FEATURE commit sha via a FOLLOW-UP commit (never amend — amend orphans the sha, fresh CI checkout can't find it). \`arbiter check tdd ${it.issue}\` must PASS. \`git add -f\` the evidence (it is under gitignored .arbiter/).
...
STOP CONDITIONS (fail-closed — do NOT fake green): an invariant you cannot satisfy, a needed human/product decision, an external dependency (credentials, paid service, publish), or a material final-review finding that remains unresolved after the bounded rework contract. Then: gh issue edit ${it.issue} --add-label needs-human, post a concise blocker-report comment, and report status="needs-human".
