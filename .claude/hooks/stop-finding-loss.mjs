...
function parseTranscript(transcriptPath) {
  ...
  try {
    ...
    // FAIL-OPEN-INTENT: transcript path missing/unreadable — treat as "no signal" so the caller stands down instead of crashing the Stop hook.
  } catch {
    return null
  }
  let dispatchCount = 0
  ...
  for (const line of raw.split('\n')) {
    ...
    try {
      ...
      // FAIL-OPEN-INTENT: a partially-written trailing transcript line is expected during a live session; skip it rather than aborting the whole parse.
    } catch {
      continue
    }
...
function countAgentReturnsSince(root, sinceMs) {
    ...
    for (const entry of readdirSync(d, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) {
        ...
        try {
          ...
          // FAIL-OPEN-INTENT: the file can be removed between readdir and stat (concurrent writer); skip it rather than aborting the walk.
        } catch {
          void 0
        }
      }
  ...
  try {
    walk(dir)
    // FAIL-OPEN-INTENT: a directory scan error (permissions/race) must not crash the Stop hook — return the partial count so far.
  } catch {
    void 0
  }
  return count
...
function main() {
  ...
  try {
    ...
    // FAIL-OPEN-INTENT: unreadable/non-JSON stdin — same stand-down posture as stop-evidence-guard.mjs; a guard that cannot read the Stop payload must not block the stop.
  } catch {
    process.exit(0)
  }
...
// Top-level guard: an unexpected crash in main() must not silently fall through to the
// shell's default success code with no explanation, nor hard-block a stop this advisory
// hook was never meant to gate — surface the error to stderr and exit 0 (fail-open).
try {
  main()
} catch (err) {
  process.stderr.write(
    `[arbiter] FINDING LOSS: unexpected error — ${err instanceof Error ? err.message : String(err)}\n`,
  )
  // FAIL-OPEN-INTENT: a crash in this advisory hook must not hard-block the Stop it does not own; surface and stand down.
  process.exit(0)
}
