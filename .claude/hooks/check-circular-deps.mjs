...
const result = spawnSync(
  madgeBin,
  ["--circular", "--extensions", "ts,tsx,js,jsx", srcDir],
  { encoding: "utf-8", cwd: repoRoot, shell: false },
);

if (result.error) {
  process.stderr.write(
    `[arbiter] circular-deps: failed to spawn madge: ${result.error.message}\n`,
  );
  process.exit(0);
}
...
if (result.signal) {
  process.stderr.write(
    `[arbiter] circular-deps: madge killed by signal ${result.signal}\n`,
  );
  process.exit(0);
}
...
if (result.status !== 0 || circularLines.length > 0) {
  ...
  if (!circularLines.length && output)
    process.stderr.write(`  ${output.slice(0, 400)}\n`);
  process.exit(2);
}
