...
// #2399 — the review-evidence binding (ancestor + source-unchanged). Loaded the same
// lazy, fail-CLOSED way, and kept a SEPARATE binding so one missing module cannot null
// both verifiers.
...
function main() {
  ...
  const dispatchStaleness = evidenceBinding.evidenceStaleness(root, dispatch.sha, {
  ...
  const journey = readJson(
    join(root, '.arbiter', 'evidence', 'journey', sanitizeTaskId(taskId) + '.json'),
  )
  if (journey === null)
    fail(
      'journey evidence missing or unreadable (.arbiter/evidence/journey/<taskId>.json) — declare the acceptance E2E spec up front and run it before claiming completion',
    )
  if (journey.branch !== branch)
    fail(`journey evidence is for branch ${JSON.stringify(journey.branch)}, not ${branch}`)
  if (!isAncestor(journey.sha))
    fail(
      'journey evidence sha is not an ancestor of HEAD (stale) — re-run the acceptance spec on the current tree',
    )
  if (typeof journey.spec !== 'string' || journey.spec.length === 0)
    fail('journey evidence does not name the acceptance spec it ran (missing "spec")')
  // Artifact parity: the run must have exercised the built artifact, never dev-server-only.
  if (journey.target !== 'artifact')
    fail(
      `journey evidence ran against ${JSON.stringify(journey.target)}, not the built artifact — a dev-server run does not prove the packaged image works (set target:"artifact" after running the spec against the compose image / dist)`,
    )
