...
describe('mergeSettingsJson', () => {
    ...
    const incoming = {
      hooks: {
        PreToolUse: [
          ...
          { matcher: 'Edit', hooks: [{ type: 'command', command: 'edit-hook.sh' }] },
        ],
      },
    };
    const result = mergeSettingsJson(existing, incoming) as Record<string, unknown>;
    const hooks = result.hooks as Record<string, { matcher: string; hooks: { command: string }[] }[]>;
  ...
  it('does not duplicate hooks with same command', () => {
    const existing = {
      hooks: {
        PreToolUse: [
          { matcher: 'Bash', hooks: [{ type: 'command', command: 'stop.sh' }] },
        ],
      },
    };
    const incoming = {
  ...
  it('unions permission arrays', () => {
    ...
    const incoming = { permissions: { allow: ['read', 'write'], deny: ['admin', 'delete'] } };
    const result = mergeSettingsJson(existing, incoming) as Record<string, unknown>;
  ...
  it('handles empty existing object', () => {
    const incoming = { hooks: { PreToolUse: [{ matcher: 'Bash', hooks: [{ type: 'command', command: 'x.sh' }] }] } };
