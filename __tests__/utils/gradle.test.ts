...
describe('injectGradleWiring', () => {
  ...
  it('places a new plugins block after a mixed line and block-comment prologue', () => {
    const p = ktsBuild(
      '// generated by build tooling\n/*\n * legacy note\n */\nimport java.time.Instant\n\nrepositories { mavenCentral() }\n',
    )
    injectGradleWiring(dir, false, { plugins: [{ id: 'checkstyle' }] })
    const out = readFileSync(p, 'utf-8')

    expect(out.indexOf('plugins {')).toBeGreaterThan(out.indexOf('import java.time.Instant'))
    expect(out.indexOf('plugins {')).toBeLessThan(out.indexOf('repositories'))
    expect(out).toContain('id("checkstyle")')
  })
  ...
  it('preserves Kotlin DSL indentation while adding missing plugins to a multi-line block', () => {
    const p = ktsBuild('plugins {\n  java\n}\n')
    injectGradleWiring(dir, false, {
      plugins: [{ id: 'checkstyle' }, { id: 'com.diffplug.spotless', version: '6.25.0' }],
    })
