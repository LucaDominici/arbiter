...
describe('detectModules', () => {
  ...
  it('typescript: detects workspaces from package.json', () => {
    ...
    writeFileSync(join(dir, 'packages/core/package.json'), JSON.stringify({ name: '@proj/core' }))
    writeFileSync(join(dir, 'packages/ui/package.json'), JSON.stringify({ name: '@proj/ui' }))
  ...
  it('typescript: handles workspaces as object with packages key', () => {
    ...
    writeFileSync(join(dir, 'packages/pkg-a/package.json'), JSON.stringify({ name: 'pkg-a' }))
    const mods = detectModules(dir, 'typescript')
