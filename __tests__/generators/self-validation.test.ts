...
describe('generateSelfValidation', () => {
  ...
  it('emits all three gate scripts', () => {
    ...
    expect(paths.some((p) => p.includes('check-exit-code-contract.mjs'))).toBe(true)
    expect(paths.some((p) => p.includes('check-pipe-tee-hazard.mjs'))).toBe(true)
  ...
  it('emitted script has a shebang line', () => {
    ...
    const content = readFileSync(join(dir, 'scripts', 'self-validation.mjs'), 'utf-8')
