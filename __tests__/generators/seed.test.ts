...
describe('generateSeed (#221)', () => {
  ...
  it('emits seed-test-data.sh and seed-common.sh for backend-web-db L2', () => {
    ...
    const result = generateSeed(config)
    expect(result.files.some((f) => f.path.endsWith('seed-test-data.sh'))).toBe(true)
    expect(result.files.some((f) => f.path.endsWith('seed-common.sh'))).toBe(true)
    expect(existsSync(join(dir, 'scripts', 'seed-test-data.sh'))).toBe(true)
    expect(existsSync(join(dir, 'scripts', 'lib', 'seed-common.sh'))).toBe(true)
  })
