...
describe('generateBehavioralTests', () => {
  ...
  it('generates BDD step definitions for typescript', () => {
    ...
    generateBehavioralTests(config)
    expect(existsSync(join(dir, 'features', 'step_definitions', 'example.steps.ts'))).toBe(true)
  })
  ...
  it('generates behavioral test example for typescript', () => {
    ...
    generateBehavioralTests(config)
    expect(existsSync(join(dir, 'src', 'test', 'example.behavioral.test.ts'))).toBe(true)
  })
  ...
  it('typescript example contains describe and it', () => {
    ...
    generateBehavioralTests(config)
    const content = readFileSync(join(dir, 'src', 'test', 'example.behavioral.test.ts'), 'utf-8')
  ...
  it('generates BDD feature file for java', () => {
    ...
    try {
      ...
      generateBehavioralTests(config)
      expect(
        existsSync(join(javaDir, 'src', 'test', 'resources', 'features', 'example.feature')),
      ).toBe(true)
    } finally {
      cleanupTestProject(javaDir)
    }
  ...
  it('generates ExampleBehavioralTest.java for java', () => {
    ...
    try {
      ...
      generateBehavioralTests(config)
      expect(
        existsSync(join(javaDir, 'src', 'test', 'java', 'example', 'ExampleBehavioralTest.java')),
      ).toBe(true)
    } finally {
      cleanupTestProject(javaDir)
    }
  ...
  it('java example contains @Nested and @DisplayName', () => {
    ...
    try {
      ...
      const content = readFileSync(
        join(javaDir, 'src', 'test', 'java', 'example', 'ExampleBehavioralTest.java'),
        'utf-8',
      )
  ...
  it('generates BDD feature file for rust', () => {
    ...
    try {
      ...
      generateBehavioralTests(config)
      expect(existsSync(join(rustDir, 'tests', 'features', 'example.feature'))).toBe(true)
    } finally {
      cleanupTestProject(rustDir)
    }
  })
  ...
  it('generates BDD test file for rust', () => {
    ...
    try {
      ...
      generateBehavioralTests(config)
      expect(existsSync(join(rustDir, 'tests', 'example_bdd_test.rs'))).toBe(true)
    } finally {
      cleanupTestProject(rustDir)
    }
  })
  ...
  it('generates example_behavioral_test.rs for rust', () => {
    ...
    try {
      ...
      generateBehavioralTests(config)
      expect(existsSync(join(rustDir, 'tests', 'example_behavioral_test.rs'))).toBe(true)
    } finally {
      cleanupTestProject(rustDir)
    }
  })
  ...
  it('rust example contains mod tests', () => {
    ...
    try {
      ...
      generateBehavioralTests(config)
      const content = readFileSync(join(rustDir, 'tests', 'example_behavioral_test.rs'), 'utf-8')
  ...
  it('generates BDD test file for go', () => {
    ...
    try {
      ...
      generateBehavioralTests(config)
      expect(existsSync(join(goDir, 'internal', 'bdd', 'example_test.go'))).toBe(true)
    } finally {
      cleanupTestProject(goDir)
    }
  })
  ...
  it('generates example_behavioral_test.go for go', () => {
    ...
    try {
      ...
      generateBehavioralTests(config)
      expect(existsSync(join(goDir, 'internal', 'example_behavioral_test.go'))).toBe(true)
    } finally {
      cleanupTestProject(goDir)
    }
  })
  ...
  it('go example contains t.Run with given/when/then naming', () => {
    ...
    try {
      ...
      generateBehavioralTests(config)
      const content = readFileSync(join(goDir, 'internal', 'example_behavioral_test.go'), 'utf-8')
  ...
  it('generates BDD feature file for python', () => {
    ...
    try {
      ...
      generateBehavioralTests(config)
      expect(existsSync(join(pyDir, 'tests', 'bdd', 'features', 'example.feature'))).toBe(true)
    } finally {
      cleanupTestProject(pyDir)
    }
  })
  ...
  it('generates BDD test file for python', () => {
    ...
    try {
      ...
      generateBehavioralTests(config)
      expect(existsSync(join(pyDir, 'tests', 'bdd', 'test_example_bdd.py'))).toBe(true)
    } finally {
      cleanupTestProject(pyDir)
    }
  })
  ...
  it('generates test_example_behavioral.py for python', () => {
    ...
    try {
      ...
      generateBehavioralTests(config)
      expect(existsSync(join(pyDir, 'tests', 'test_example_behavioral.py'))).toBe(true)
    } finally {
      cleanupTestProject(pyDir)
    }
  })
  ...
  it('python example contains class and descriptive test methods', () => {
    ...
    try {
      ...
      generateBehavioralTests(config)
      const content = readFileSync(join(pyDir, 'tests', 'test_example_behavioral.py'), 'utf-8')
  ...
  it('TESTING_POLICY.md contains mock policy section', () => {
    ...
    generateBehavioralTests(config)
    const content = readFileSync(join(dir, 'docs', 'TESTING_POLICY.md'), 'utf-8')
  ...
  it('eslint-playwright.json contains no-force-option rule', () => {
    ...
    generateBehavioralTests(config)
    const content = readFileSync(join(dir, '.eslintrc-playwright.json'), 'utf-8')
  ...
  it('check-test-naming.mjs for typescript checks .test.ts pattern', () => {
    ...
    generateBehavioralTests(config)
    const content = readFileSync(join(dir, 'scripts', 'check-test-naming.mjs'), 'utf-8')
  ...
  it('check-test-naming.mjs for java checks Test.java pattern', () => {
    ...
    try {
      ...
      generateBehavioralTests(config)
      const content = readFileSync(join(javaDir, 'scripts', 'check-test-naming.mjs'), 'utf-8')
  ...
  it('check-test-naming.mjs for python checks test_ pattern', () => {
    ...
    try {
      ...
      generateBehavioralTests(config)
      const content = readFileSync(join(pyDir, 'scripts', 'check-test-naming.mjs'), 'utf-8')
  ...
  it('does not emit java files for typescript', () => {
    ...
    generateBehavioralTests(config)
    expect(
      existsSync(join(dir, 'src', 'test', 'java', 'example', 'ExampleBehavioralTest.java')),
    ).toBe(false)
  })
