...
function runCapture(dir: string, args: string[] = []) {
  const r = spawnSync('node', [join(dir, 'scripts', 'capture-debt-baseline.mjs'), ...args], {
    cwd: dir,
    encoding: 'utf-8',
  })
  if (r.status !== 0) {
...
function readBaseline(dir: string) {
  return JSON.parse(readFileSync(join(dir, 'scripts', 'debt-baseline.json'), 'utf-8'))
}
...
describe('capture-debt-baseline --update: missing-tool preservation (#126)', () => {
  ...
  beforeEach(() => {
    ...
    writeFileSync(join(dir, 'scripts', 'capture-debt-baseline.mjs'), renderCapture(), 'utf-8')
  })
  ...
  it('malformed baseline file produces error message including filename', () => {
    writeFileSync(join(dir, 'scripts', 'debt-baseline.json'), '{ this is not valid json', 'utf-8')
    writeFakeDebtLib(dir, {
      tool_a: { value: 70, unit: 'percent', direction: 'higher-is-better' },
    })

    const r = spawnSync('node', [join(dir, 'scripts', 'capture-debt-baseline.mjs'), '--update'], {
      cwd: dir,
      encoding: 'utf-8',
    })
