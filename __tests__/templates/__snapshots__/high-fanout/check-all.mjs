...
// separate cache.
if (!process.env.TMPDIR && !process.argv.includes('--dry-run')) {
...
const isCIValue = (value) => ['1', 'true', 'yes', 'on'].includes((value ?? '').trim().toLowerCase());
const IS_CI = isCIValue(process.env.CI) || isCIValue(process.env.GITHUB_ACTIONS);
...
const _gateStart = dryRun ? null : await (async () => {
  try {
...
if (dryRun) {
  ...
  if (existsSync('scripts/debt-baseline.json')) {
    try {
      ...
      _debtThresholds = Object.entries(_debtMetrics).map(([_name, _metric]) => ({
        name: _name,
        value: _metric.value,
      ...
      _debtContractReason = `debt threshold authority is unreadable: ${err.message}`;
    }
  }

  const _contractGates = GATE_REGISTRY
    .filter((_g) => _LEVELS.indexOf(_g.level) <= _LEVELS.indexOf(level))
    .map((_g) => ({
      name: _g.name,
      id: _g.id,
      enforcement: _g.kind === 'warn' ? 'advisory' : _g.kind === 'tool' ? 'tool' : 'hard',
      command: _g.cmd ? _g.cmd.join(' ') : _g.inspect,
      condition: _g.condition ?? 'selected gate level is active',
      ...(_g.id === 'debt-ratchet' ? { thresholds: _debtThresholds } :
        _g.thresholds ? { thresholds: _g.thresholds } : {}),
      ...(_g.id === 'debt-ratchet' ? {
        bindings: [
  ...
  const _workflowAuthority = existsSync(_workflowDir)
    ? readdirSync(_workflowDir)
      .filter((_name) => /\.ya?ml$/.test(_name))
      .map((_name) => join(_workflowDir, _name))
    : [];
  const _contractAuthorityPaths = [
  ...
  const _deletedConditionBindings = _conditionBindingGroups
    ...
    .map((_binding) => _binding.source);
  if (true && _workflowAuthority.length === 0) {
    _missingContractAuthority.push(_workflowDir);
  }
  const _contractAuthority = [...new Set(_contractAuthorityPaths)]
    .filter((_path) => existsSync(_path))
    .map((_path) => ({
      path: _path,
  ...
  console.log(JSON.stringify({
    ...
    gates: _contractGates,
    external: _workflowContract.external,
    unresolved: [...new Set(_missingContractAuthority)].map((_path) => ({
    ...
    }] : []).concat(_workflowContract.unresolved),
  }, null, 2));
  process.exit(0);
}
if (only !== null) {
...
runCheck('complexity ratchet (preventive)', 'node', ['scripts/debt-report.mjs', '--gate', '--only-metric', 'complexityViolations']);
...
runCheck('orphan TODOs (INV-21)', 'node', ['scripts/check-no-orphan-todo.mjs']);
...
const _presuitePassed = getFailed() === 0;
if (!_preflight && (_presuitePassed || dryRun || only !== null)) {
...
// `import()` of a .mjs module, then a child-process + nonce handoff) were
// both defeated by Codex review because ANY in-process trust placed in a
// LOADED CODE MODULE is forgeable — a top-level `process.exit(0)` green-exits
// the gate if imported in-process; a child-process handoff still leaks its
// secret via `process.execArgv` (the `-e` source is visible there) and its
// file-write channel is interceptable via `syncBuiltinESMExports`. A JSON
// file has no code to run and nothing to forge: `JSON.parse` either returns
// data or throws, with no third option and no way for the data to act.
//
// Shape: { "checks": [{ "name": string, "cmd": [string, ...], "tier": "L1"|"L2"|"L3"|"L4" }] }
// Absent file is a plain no-op skip (mirrors gateFileState's documented
// 'never-emitted' skip below) — most projects never create this file.
// Unreadable, invalid JSON, a non-object top level, a non-array `checks`, or
// a malformed entry (missing name/cmd/a valid tier, or a cmd containing a
// non-string/empty-string element) FAILS LOUD per INV-96 (fail-closed on
// uncertainty) instead of being silently dropped.
//
...
// (AC3); check-local-ci-parity.mjs compares that JSON's parityContentHash, so
// a local check's PASS/FAIL flows into local↔CI parity the same way.
//
// Runs once, after every lane: a local check's own `tier` gates it, not this
