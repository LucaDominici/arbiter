...
const STACK_CONFIGS: Record<Language, Partial<Parameters<typeof makeConfig>[1]>> = {
  typescript: {
    buildTool: 'npm',
...
function configFor(lang: Language, level: GovernanceLevel = 'L2'): Record<string, unknown> {
...
describe('hooks/pre-compact.mjs.ejs', () => {
  it('renders without EJS tag leaks', () => {
    const out = renderTemplate('claude/hooks/pre-compact.mjs.ejs', configFor('typescript'))
  ...
  it('renders identically for all stacks (no stack interpolation)', () => {
    const base = renderTemplate('claude/hooks/pre-compact.mjs.ejs', configFor('typescript'))
    for (const lang of ['java', 'rust', 'go', 'python'] as Language[]) {
      const out = renderTemplate('claude/hooks/pre-compact.mjs.ejs', configFor(lang))
...
describe('hooks/pre-edit-plan-anchor.mjs.ejs', () => {
  ...
  it('renders identically for all stacks', () => {
    ...
    for (const lang of ['java', 'rust', 'go', 'python'] as Language[]) {
      const out = renderTemplate('claude/hooks/pre-edit-plan-anchor.mjs.ejs', configFor(lang))
      expect(out).toBe(base)
    }
  })
})
...
describe('hooks/skill-forced-eval.mjs.ejs', () => {
  ...
  it('renders without EJS tag leaks for all stacks', () => {
    for (const lang of LANGUAGES) {
      const out = renderTemplate('claude/hooks/skill-forced-eval.mjs.ejs', configFor(lang))
      expect(out).not.toContain('<%')
      expect(out).not.toContain('%>')
    }
  })
  ...
  it('interpolates testCommand for rust', () => {
    const out = renderTemplate('claude/hooks/skill-forced-eval.mjs.ejs', configFor('rust'))
    expect(out).toContain('cargo test')
  })

  it('interpolates testCommand for python', () => {
    const out = renderTemplate('claude/hooks/skill-forced-eval.mjs.ejs', configFor('python'))
...
describe('hooks/debug-state-on-failure.mjs.ejs', () => {
  ...
  it('renders without EJS tag leaks for all stacks', () => {
    for (const lang of LANGUAGES) {
      const out = renderTemplate('claude/hooks/debug-state-on-failure.mjs.ejs', configFor(lang))
      expect(out).not.toContain('<%')
      expect(out).not.toContain('%>')
    }
  })
  ...
  it('includes rust test command pattern', () => {
    const out = renderTemplate('claude/hooks/debug-state-on-failure.mjs.ejs', configFor('rust'))
    expect(out).toContain('cargo test')
  })

  it('includes python test command pattern', () => {
    const out = renderTemplate('claude/hooks/debug-state-on-failure.mjs.ejs', configFor('python'))
    expect(out).toContain('pytest')
  })

  it('includes java test command pattern', () => {
    const out = renderTemplate('claude/hooks/debug-state-on-failure.mjs.ejs', configFor('java'))
...
describe('hooks/post-edit-dispatch.mjs.ejs', () => {
  ...
  it('renders without EJS tag leaks for all stacks', () => {
    for (const lang of LANGUAGES) {
      const out = renderTemplate('claude/hooks/post-edit-dispatch.mjs.ejs', configFor(lang))
      expect(out).not.toContain('<%')
      expect(out).not.toContain('%>')
    }
  })
  ...
  it('rust: contains cargo fmt formatCommand', () => {
    const out = renderTemplate('claude/hooks/post-edit-dispatch.mjs.ejs', configFor('rust'))
  ...
  it('go: contains gofmt formatCommand', () => {
    const out = renderTemplate('claude/hooks/post-edit-dispatch.mjs.ejs', configFor('go'))
    expect(out).toContain('gofmt')
  })

  it('python: contains black formatCommand', () => {
    const out = renderTemplate('claude/hooks/post-edit-dispatch.mjs.ejs', configFor('python'))
...
describe('hooks/guard-task-completion.mjs.ejs', () => {
  ...
  it('renders without EJS tag leaks for all stacks', () => {
    for (const lang of LANGUAGES) {
      const out = renderTemplate('claude/hooks/guard-task-completion.mjs.ejs', configFor(lang))
      expect(out).not.toContain('<%')
      expect(out).not.toContain('%>')
    }
  })
  ...
  it('interpolates testCommand for rust', () => {
    const out = renderTemplate('claude/hooks/guard-task-completion.mjs.ejs', configFor('rust'))
    expect(out).toContain('cargo test')
  })
...
describe('hooks/check-circular-deps.mjs.ejs (#167)', () => {
  it('renders without EJS leaks for TypeScript', () => {
    const out = renderTemplate('claude/hooks/check-circular-deps.mjs.ejs', configFor('typescript'))
    expect(out).not.toContain('<%')
    expect(out).not.toContain('%>')
  })
...
describe('generateClaude — check-circular-deps.mjs emission (#167)', () => {
  it('emits check-circular-deps.mjs for TypeScript projects', () => {
    ...
    try {
      ...
      expect(paths.some((p) => p.endsWith('check-circular-deps.mjs'))).toBe(true)
    } finally {
      rmSync(dir, { recursive: true, force: true })
    }
  })
  ...
  for (const lang of ['java', 'rust', 'go', 'python'] as Language[]) {
    it(`does NOT emit check-circular-deps.mjs for ${lang} projects`, () => {
      ...
      try {
        ...
        const result = generateClaude(config)
        const paths = result.files.map((f) => f.path)
        expect(paths.some((p) => p.endsWith('check-circular-deps.mjs'))).toBe(false)
      } finally {
        rmSync(dir, { recursive: true, force: true })
      }
    })
