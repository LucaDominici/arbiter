...
function renderHeartbeat(overrides: Record<string, unknown> = {}) {
  return renderTemplate(
    'github/workflows/08-heartbeat.yml.ejs',
...
describe('08-heartbeat.yml.ejs — structural invariants (CANON-18)', () => {
  const STACKS = [
...
describe('08-heartbeat.yml.ejs — stack independence', () => {
...
describe('08-heartbeat.yml.ejs — trigger and schedule', () => {
...
describe('08-heartbeat.yml.ejs — freshness threshold assertions', () => {
  ...
  it('weekly threshold is 8 days', () => {
    ...
    expect(weeklySection).toContain('8')
  })
...
describe('08-heartbeat.yml.ejs — issue filing on failure', () => {
  ...
  it('files issue on weekly freshness failure', () => {
    const rendered = renderHeartbeat({})
    const weeklySection = rendered.split('assert-weekly-freshness:')[1] ?? ''
    expect(weeklySection).toContain('gh issue create')
    expect(weeklySection).toContain('heartbeat-weekly-missed')
  })
  ...
  it('issue filing steps run only on failure', () => {
    ...
    expect(issueSteps).toBeGreaterThanOrEqual(2)
  })
})
