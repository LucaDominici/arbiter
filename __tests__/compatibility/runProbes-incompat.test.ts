...
describe('runProbes — incompat-gradle-pitest fixture', () => {
  it('detects java stack from build.gradle', () => {
    ...
    mockRunCli.mockImplementation((cmd: string) => {
      ...
      if (cmd === 'gradle') return { stdout: 'Gradle 8.5', stderr: '', exitCode: 0, durationMs: 5 }
      if (cmd === 'mvn')
        return {
  ...
  it('hasFailures=true when Gradle version below >=7', () => {
    mockRunCli.mockImplementation((cmd: string) => {
      ...
      if (cmd === 'gradle') return { stdout: 'Gradle 6.9', stderr: '', exitCode: 0, durationMs: 5 }
      if (cmd === 'mvn')
        return {
          stdout: 'Apache Maven 3.9.6',
          stderr: '',
      ...
      if (cmd === './gradlew') return { stdout: '', stderr: '', exitCode: 0, durationMs: 5 }
      return { stdout: '', stderr: '', exitCode: 0, durationMs: 1 }
    })
    const report = runProbes(FIXTURE_DIR)
