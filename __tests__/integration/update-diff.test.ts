...
describe('arbiter update', () => {
  ...
  it('update never emits a retired tool file, and is not bricked by a legacy config that names one', async () => {
    // #2367 (ADR-119): cursor/copilot/gemini/windsurf/aider were RETIRED — the
    // generators and templates are deleted, so `update` has nothing to emit for
    // them. A legacy arbiter.json that still lists them must NOT brick: the
    // never-brick coercion (ADR-105, sanitizeCoercibleFields) filters the
    // unknown entries, and the claude/codex trees regenerate as usual.
    const configPath = join(dir, 'arbiter.json')
    const config = JSON.parse(readFileSync(configPath, 'utf-8')) as { tools: string[] }
    config.tools = ['claude', 'codex', 'cursor', 'copilot', 'gemini', 'windsurf', 'aider']
    writeFileSync(configPath, JSON.stringify(config, null, 2), 'utf-8')

    await runUpdate({ dir, github: false })

    // The supported trees still regenerate — the legacy config is usable.
    expect(existsSync(join(dir, 'AGENTS.md'))).toBe(true)
    expect(existsSync(join(dir, '.claude', 'CLAUDE.md'))).toBe(true)
    expect(existsSync(join(dir, '.agents', 'CODEX.md'))).toBe(true)

    // Nothing is emitted for any retired tool.
    expect(existsSync(join(dir, '.cursorrules'))).toBe(false)
    expect(existsSync(join(dir, '.github', 'copilot-instructions.md'))).toBe(false)
    expect(existsSync(join(dir, '.gemini', 'GEMINI.md'))).toBe(false)
    expect(existsSync(join(dir, 'windsurf-instructions.md'))).toBe(false)
