...
describe('brownfield: existing .claude/ directory', () => {
  ...
  function configWithExistingClaude(extraExisting: Partial<Parameters<typeof makeConfig>[1]['existing']> = {}) {
  ...
  it('preserves custom rules (skipIfExists)', () => {
    ...
    const content = readFileSync(join(rulesDir, '05-agent-lifecycle.md'), 'utf-8');
