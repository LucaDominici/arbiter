...
describe("impactedGenerators — full regen triggers", () => {
  ...
  it("archetype change → *", () => {
    const diff = diffConfig(
      baseV2({ archetype: "library" }),
      baseV2({ archetype: "cli" }),
    );
    const keys = impactedGenerators(diff);
    expect(keys.has("*")).toBe(true);
  });
  ...
  it("isMultiTenant change → *", () => {
    const diff = diffConfig(
      baseV2({ isMultiTenant: false }),
      baseV2({ isMultiTenant: true }),
    );
    expect(impactedGenerators(diff).has("*")).toBe(true);
  });
  ...
  it("hasDatabase change → *", () => {
    const diff = diffConfig(
      baseV2({ hasDatabase: false }),
      baseV2({ hasDatabase: true }),
    );
    expect(impactedGenerators(diff).has("*")).toBe(true);
  });
  ...
  it("hasPublicApi change → *", () => {
    const diff = diffConfig(
      baseV2({ hasPublicApi: false }),
      baseV2({ hasPublicApi: true }),
    );
    expect(impactedGenerators(diff).has("*")).toBe(true);
  });
  ...
  it("contractType change → *", () => {
    const diff = diffConfig(
      baseV2({ contractType: "none" }),
      baseV2({ contractType: "grpc" }),
    );
    expect(impactedGenerators(diff).has("*")).toBe(true);
  });
});
