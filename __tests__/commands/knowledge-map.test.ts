...
function makeDir(): { dir: string; cleanup: () => void } {
  ...
  writeFileSync(join(dir, 'scripts', 'knowledge-map-update.mjs'), `#!/usr/bin/env node\n`)
...
describe('runKnowledgeMapUpdate (#255)', () => {
  ...
  it('runs knowledge-map-update.mjs script', async () => {
    ...
    try {
      const { runKnowledgeMapUpdate } = await import('../../src/commands/knowledge-map.js')
      runKnowledgeMapUpdate({ dir })
      expect(runCliMock).toHaveBeenCalledTimes(1)
      const scriptArg = String((runCliMock.mock.calls[0] as unknown[][])[1]?.[0] ?? '')
