...
describe("runWorktreeList", () => {
  ...
  it("emits 'No open task worktrees' when list is empty (main only)", async () => {
    ...
    mockRunCli
      ...
      .mockReturnValueOnce(ok(porcelain));
...
describe("runWorktreeOpen", () => {
  ...
  it("throws when not running from main repo", async () => {
    ...
    ).toThrow("Must run from the main repository");
  });
  ...
  it("opens worktree and writes log entry on success", async () => {
    ...
    expect(logSpy).toHaveBeenCalledWith(
      expect.stringContaining("Worktree ready"),
    );
    logSpy.mockRestore();
  });
...
describe("runWorktreeClose", () => {
  ...
  it("throws when not running from main repo", async () => {
    ...
    ).toThrow("Must run 'worktree close' from the main repository");
  });

  it("throws when no open worktree found for task", async () => {
    const { runWorktreeClose } = await import("../../src/commands/worktree.js");
    expect(() =>
      runWorktreeClose({ taskId: "999", cwd: gitRoot, noFetch: true }),
    ).toThrow("No open worktree found for task");
  });
  ...
  it("closes successfully when branch is merged", async () => {
    ...
    expect(logSpy).toHaveBeenCalledWith(
      expect.stringContaining("Worktree closed"),
    );
    logSpy.mockRestore();
  });
  ...
  it("emits warning for stale log entry and throws", async () => {
    ...
    const { runWorktreeClose } = await import("../../src/commands/worktree.js");
    expect(() =>
      runWorktreeClose({ taskId: "456", cwd: gitRoot, noFetch: true }),
    ).toThrow("No open worktree found");
    expect(stderrSpy).toHaveBeenCalledWith(
      expect.stringContaining("Worktree directory missing"),
    );
