...
describe('matrix: Java project', () => {
  ...
  it('does not include TypeScript-specific hooks', () => {
    const config = javaConfig()
    runGenerators(config)
    expect(existsSync(join(dir, '.claude', 'hooks', 'check-no-any.mjs'))).toBe(false)
  })
...
describe('matrix: Java project (Maven)', () => {
  ...
  function mavenConfig(overrides: Partial<Parameters<typeof makeConfig>[1]> = {}) {
    return makeConfig(dir, {
      language: 'java',
      framework: null,
      buildTool: 'maven',
  ...
  it('dependabot.yml includes maven ecosystem', () => {
    const config = mavenConfig()
    runGenerators(config)
    const dependabot = readFileSync(join(dir, '.github', 'dependabot.yml'), 'utf-8')
...
describe('matrix: Java project — hexagonal architecture suite (M22)', () => {
  ...
  function hexConfig(overrides: Partial<Parameters<typeof makeConfig>[1]> = {}) {
  ...
  it('emits arch-test-deps.gradle fragment', () => {
    const config = hexConfig()
    runGenerators(config)
    expect(existsSync(join(hexDir, 'gradle', 'arch-test-deps.gradle'))).toBe(true)
  })
  ...
  it('check-all.mjs includes architecture tests step for hexagonal Gradle', () => {
    const config = hexConfig()
    runGenerators(config)
    const content = readFileSync(join(hexDir, 'scripts', 'check-all.mjs'), 'utf-8')
...
describe('matrix: Java L3 mutation gate (pitest)', () => {
  ...
  function javaL3Config(overrides: Partial<Parameters<typeof makeConfig>[1]> = {}) {
    return makeConfig(dir, {
      language: 'java',
      governanceLevel: 'L3',
      buildTool: 'gradle',
  ...
  it('check-all.mjs invokes pitest at L3 (#347 — INV-30 wired in L2 block, runs at L2+)', () => {
    const config = javaL3Config()
    runGenerators(config)
