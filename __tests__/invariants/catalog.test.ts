...
describe("INVARIANT_CATALOG", () => {
  it("has exactly 31 entries", () => {
    expect(INVARIANT_CATALOG).toHaveLength(31);
  });
  ...
  it("has exactly 4 Tier 2 invariants", () => {
  ...
  it("has exactly 6 Tier 4 invariants", () => {
    const tier4 = INVARIANT_CATALOG.filter((inv) => inv.tier === "operational");
    expect(tier4).toHaveLength(6);
  });

  it("has exactly 9 Tier 5 invariants", () => {
    const tier5 = INVARIANT_CATALOG.filter((inv) => inv.tier === "governance");
    expect(tier5).toHaveLength(9);
  });
...
describe("getFilteredInvariants", () => {
  ...
  it("returns 29 for TypeScript + L3 + all tiers (Java-only invariants excluded)", () => {
    const result = getFilteredInvariants({
      language: "typescript",
      governanceLevel: "L3",
      invariantTiers: ALL_TIERS,
  ...
  it("Java + L2 + all tiers returns 29 invariants (L3-gated INV-27/28 excluded)", () => {
    const result = getFilteredInvariants({
      language: "java",
      governanceLevel: "L2",
      invariantTiers: ALL_TIERS,
  ...
  it("Java + L3 + all tiers returns all 31 invariants", () => {
    const result = getFilteredInvariants({
      language: "java",
      governanceLevel: "L3",
      invariantTiers: ALL_TIERS,
