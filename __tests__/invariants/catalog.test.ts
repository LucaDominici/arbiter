...
describe('INVARIANT_CATALOG', () => {
  it('has exactly 82 entries', () => {
    expect(INVARIANT_CATALOG).toHaveLength(82)
  })
  ...
  it('has exactly 19 Tier 4 invariants', () => {
    const tier4 = INVARIANT_CATALOG.filter((inv) => inv.tier === 'operational')
    expect(tier4).toHaveLength(19)
  })
...
describe('getFilteredInvariants', () => {
  ...
  it('returns 55 for TypeScript + L3 + all tiers (INV-29/30/44 Java-only + 11 selfOnly excluded)', () => {
    const result = getFilteredInvariants({
      language: 'typescript',
      governanceLevel: 'L3',
      invariantTiers: ALL_TIERS,
  ...
  it('Java + L2 + all tiers returns 54 invariants (L3-gated INV-27/28/33 + 11 selfOnly excluded)', () => {
    const result = getFilteredInvariants({
      language: 'java',
      governanceLevel: 'L2',
      invariantTiers: ALL_TIERS,
  ...
  it('Java + L3 + all tiers returns 57 invariants (11 selfOnly excluded)', () => {
    const result = getFilteredInvariants({
      language: 'java',
      governanceLevel: 'L3',
      invariantTiers: ALL_TIERS,
  ...
  it('catalog has exactly 13 selfOnly invariants (#682, #862)', () => {
    const selfOnly = INVARIANT_CATALOG.filter((inv) => inv.selfOnly === true)
    expect(selfOnly).toHaveLength(13)
