...
// nested two-plus directories deep — exactly haben's `docs/architecture/budget/` (arc42.md +
// c4-model.md + a docs/architecture/budget/adr/ subtree with 9 ADR-NNN_*.md records) — was
// invisible to it and reported MISSING.
//
// This fixture reproduces that real haben structure (dogfood-proof against the actual haben
// repo was additionally run manually — see the task report — but the fixture below is the
// repeatable, CI-safe regression guard).
...
function makeHabenLikeRepo(): { dir: string; cleanup: () => void } {
  ...
  // Reproduce haben's real docs/architecture/budget/ subtree.
...
describe('check-doc-set: subtree arc42/C4/ADR recognition (H2, gold-doc-capability T2)', () => {
  it('RED (pre-T2 accept_any): flat-only architecture accept_any does NOT see budget/arc42.md', () => {
    ...
    // this is exactly what shipped before T2 and is exactly why haben was reported as missing
    ...
    const { dir, cleanup } = makeHabenLikeRepo()
    try {
...
`
  ...
  // red/green comparison for the ADR side was verified directly against the real haben repo using
