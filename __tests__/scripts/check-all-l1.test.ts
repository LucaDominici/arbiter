...
describe('check-all.mjs L1 wiring', () => {
  it('keeps self preflight to immediate safety diagnostics and preventive ratchets (#2773)', () => {
    ...
    expect(block.match(/runCheck\(/g)).toHaveLength(9)
    ...
    expect(block).toContain("['scripts/check-anti-telemetry.mjs']")
    expect(block).toContain("'public API ratchet (preventive)'")
