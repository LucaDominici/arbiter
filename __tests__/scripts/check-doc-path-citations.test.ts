...
// ─── #2260 follow-up: gitignored-by-design paths are not dead ───────────────
// RED: `.claude/settings.local.json` exists on a developer's machine and NEVER
// in git, so the scan was green locally and red in a fresh CI checkout. A
// citation is dead only when the path is neither tracked NOR ignored.

describe('#2260 — a gitignored path is local-by-design, not a phantom', () => {
  const runAgainstRepo = (markdown: string) => {
    const dir = mkdtempSync(join(tmpdir(), 'path-citation-ignored-'))
    try {
      writeFileSync(join(dir, 'doc.md'), markdown)
      // cwd is the real repo so `git check-ignore` resolves against its .gitignore
      return spawnSync('node', [SCRIPT, `--roots=${dir}`], {
        encoding: 'utf-8',
        cwd: resolve('.'),
      })
    } finally {
  ...
  it('does not flag an ignored path that is absent from the checkout', () => {
    // wiki/ is gitignored (generated by gen-wiki.mjs); this page never exists.
    const r = runAgainstRepo('The wiki lands at `wiki/nonexistent-page.md`.\n')
    expect(r.stdout).not.toContain('phantom-path:')
    expect(r.status).toBe(0)
  })

  it('does not flag `.claude/settings.local.json` — the CI-only red from #2283', () => {
    const r = runAgainstRepo('Per-machine overrides live in `.claude/settings.local.json`.\n')
    expect(r.stdout).not.toContain('phantom-path:')
    expect(r.status).toBe(0)
  })
