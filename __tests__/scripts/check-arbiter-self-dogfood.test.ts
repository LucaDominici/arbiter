...
function fixtureRepo(): string {
  ...
  // Minimal repo skeleton — script reads repo root relative to its own location,
  // so we copy the script into the fixture and run from there.
  mkdirSync(join(dir, 'scripts'), { recursive: true })
  mkdirSync(join(dir, '.github', 'workflows'), { recursive: true })
  // Copy the real script verbatim so logic stays identical.
...
describe('check-arbiter-self-dogfood.mjs', () => {
  ...
  it('flags write-all as a violation', () => {
    ...
    try {
      ...
      // Baseline at 1 violation; we don't expand it below
