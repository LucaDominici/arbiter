...
> **Want the literal output instead of the narrated tour?** [`examples/go-library/`](./go-library/) is the generated, CI-verified result of step 2 below at `--level L1` (regenerated by `npm run examples:check` in `.github/workflows/generator-matrix.yml`; drift fails the build). This walkthrough narrates the fuller `--level L2` tour by hand — CI workflow, `govulncheck`, the enforcement-chain edit in §5 — and is not itself drift-checked.
