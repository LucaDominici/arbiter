...
// ts-library-fixture — docs/USE_CASES.md gate (UC-NN, INV-149)
//
...
export function extractBlock(text) {
  ...
  if (!fence) return { ok: false, error: `has no \`\`\`json fence between the ${SENTINEL} sentinels` }
  try {
...
export function findDanglingFeatures(useCases, matrixIds) {
  ...
  for (const uc of useCases) {
    for (const f of Array.isArray(uc['featureIds']) ? uc['featureIds'] : []) {
      ...
      out.push(`use case ${String(uc['id'])}: featureId "${String(f)}" is not a row in ${MATRIX_REL}`)
    }
  }
  return out
}
