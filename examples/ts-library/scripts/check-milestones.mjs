...
// ts-library-fixture — docs/MILESTONES.md gate (MS-NN, EP-NN, INV-146)
//
// Emitted twin of arbiter's scripts/check-milestones.mjs. Every RULE is identical — schema
// validity, unique ids, acyclic DAG, Now/Next/Later decay, fail-closed `done`, and the epic join
// checked in both directions. THREE things differ, and each is forced rather than chosen:
//
//   1. The SSOT is docs/MILESTONES.md carrying a fenced JSON block between sentinels, not YAML.
//      A governed project has no YAML parser: `yaml` is a dependency of arbiter, no emitted script
//      imports it, and a Go or Python project receives 77 .mjs gates and NO package.json at all —
//      there is nowhere to declare one. Shipping `import YAML from 'yaml'` here would be a gate
//      that dies on MODULE_NOT_FOUND the first time a project writes a milestone. The sentinel
//      block is not a new invention either: it is what docs/SOURCES.md and docs/USE_CASES.md
//      already use on this track, for exactly this reason.
//   2. The path is flat under docs/, as every other emitted document is.
//   3. An `INV-NN` evidence_ref resolves against AGENTS.md, which is where a governed project's
//      invariants live; the TypeScript catalog is arbiter's own.
//
// Pinned in .dogfood-divergences.json. Nothing else is scoped down.
...
//
// Asserts:
//   1. the sentinel block parses as JSON and satisfies schemas/milestone.schema.json — which is where
...
//
// A missing MILESTONES.md is a SKIP, not a violation: a project need not have codified a roadmap.
...
//
// --emit writes the machine projection a viewer consumes (schema arbiter-milestones-v1). It is
// written ONLY after every rule has passed, so an invalid milestone set cannot produce a
// projection — a property a separate generator could not offer.
//
...
const MILESTONES_REL = join('docs', 'MILESTONES.md')
const SCHEMA_REL = join('schemas', 'milestone.schema.json')
const CATALOG_REL = 'AGENTS.md'
const START = '<!-- MILESTONES_START -->'
const END = '<!-- MILESTONES_END -->'
...
/**
 * The machine projection a viewer consumes. The SSOT is a document a human edits; this is the
 * derived form a tool reads, and keeping them separate is what stops a consumer reimplementing the
 * reader and holding a second opinion about the same plan.
 *
...
/**
 * Pull the fenced JSON out of the sentinel block. THROWS on a malformed block rather than
 * returning an empty set: "this project codified no milestones" and "the file is broken" must not
 * produce the same green, which is the same rule the emitted sources gate holds itself to.
...
export function extractBlock(markdown) {
  ...
  if (start < 0 || end < 0 || end < start) {
    throw new Error(`missing its ${START} / ${END} sentinel block`)
  }
  const fence = /```json\n([\s\S]*?)\n```/.exec(markdown.slice(start + START.length, end))
  if (!fence) throw new Error(`has no \`\`\`json fence between its sentinels`)
  try {
    return JSON.parse(fence[1])
  } catch (err) {
    throw new Error(`block is not valid JSON — ${err.message}`)
  }
}
