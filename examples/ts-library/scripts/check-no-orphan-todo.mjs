...
function isRegexStart(line, index) {
  ...
  const previousChar = line[previous]
  if ((previousChar === '+' || previousChar === '-') && line[previous - 1] === previousChar) {
    return false
  }
  if ('([{,:;=!?&|+-*%^~<>'.includes(previousChar)) return true
  const prefix = line.slice(0, previous + 1)
  return /(?:^|\s)(?:return|throw|case|delete|void|typeof|instanceof|in|of|yield|await|else|do|new)\s*$/.test(prefix)
}
...
function scanTokenStart(line, index, state, tripleQuotes, rawBackticks, hashComments) {
  const char = line[index]
...
function scanCode(line, index, state, hashComments, tripleQuotes, rawBackticks, regexLiterals) {
  const token = scanTokenStart(line, index, state, tripleQuotes, rawBackticks, hashComments)
  ...
  if (regexLiterals && line[index] === '/' && isRegexStart(line, index)) {
    return scanRegex(line, index)
  }
  return { found: false, next: index + 1 }
}

function scanLine(line, state, hashComments, tripleQuotes, rawBackticks, regexLiterals) {
  let found = false
  for (let index = 0; index < line.length;) {
    ...
    if (state.blockComment) step = scanBlockComment(line, index, state)
      ...
      continue
    } else step = scanCode(line, index, state, hashComments, tripleQuotes, rawBackticks, regexLiterals)
...
export function findOrphanTodos(content, extension = '.ts') {
  ...
  const regexLiterals = ['.ts', '.tsx', '.mjs', '.js', '.jsx'].includes(normalizedExtension)
  const state = { blockComment: false, quote: null, tripleQuote: null, escaped: false, rawQuote: false }
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (scanLine(line, state, hashComments, tripleQuotes, rawBackticks, regexLiterals)) {
...
export function main(exitFn = process.exit) {
  const scanDirs =
    process.argv.slice(2).length > 0 ? process.argv.slice(2) : ["src","__tests__"]
