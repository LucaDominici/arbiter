...
if (!existsSync(madgeBin)) {
  process.stdout.write(`[arbiter] circular-deps: madge not installed — skip (run: npm install -D madge)\n`);
  process.exit(0);
}

// madge rebuilds the WHOLE src/ import graph; it cannot be scoped to one file.
// Debounce so a burst of edits triggers at most one run instead of O(repo)/edit
// (#1515). The L1 gate runs madge --circular authoritatively before commit/push,
// so a skipped per-edit run never lets a cycle reach a commit.
if (debounceHook("madge-circular-deps")) process.exit(0);
...
if (result.status !== 0 || circularLines.length > 0) {
  process.stderr.write(`[arbiter] INV-01: Circular dependency detected (triggered by ${file.replace(repoRoot + "/", "")}):\n`);
