...
- Run `/ship` to advance into delivery

The `post-brainstorm-stop` hook enforces this: it blocks delivery until the marker
...
begins only after a human picks up the issue with `/ship #<issue-number>`.
