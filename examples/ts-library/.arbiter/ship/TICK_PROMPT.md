...
- Fix-on-red is YOUR judgment from the PR's own history (no engine call): on a red gate,
  ...
  attempt the root-cause fix on the first and second strike, escalate on the third.
...
1. An open `ship` PR exists:
   ...
   c. Red → capture the failed log (`gh run view --log-failed > /tmp/red.log`), count
      ...
      - first or second strike on this gate → reproduce the failed gate locally FIRST,
      ...
      - the same gate already failed twice → post the diagnosis on the PR, label the
...
- The 2-strike rule is final: once a gate has failed twice, escalate — label `needs-human`
