...
// scripts/check-no-stub-redirects.mjs — generated by arbiter (anti-fake-green, #1497).
//
// No-stub-redirects guard. A stale "Moved → X" stub .md is a fake-green for docs: the link checker
// passes and the page "exists", but it is a redirect husk with no real content — a green doc signal
// not backed by a real doc. This guard flags a .md that is (a) a heading/leading line with a
// redirect VERB (Moved / Relocated / Renamed / has moved / see instead), (b) a short body, and
// (c) exactly one .md link. Allowlist (.stub-redirects-allowlist) entries REQUIRE a hard
// `EXPIRES: YYYY-MM-DD` date — an open-ended or lapsed exemption is itself a fake-green, so it
// fails closed. NO-DATA (no docs) is a PASS (nothing to flag), not a manufactured fail.
//
// Exit codes (INV-53): 0=PASS, 1=FAIL (un-allowlisted stub / bad allowlist), 2=ERROR.
...
function main() {
  ...
  if (violations.length > 0) {
    process.stderr.write(
      `check-no-stub-redirects: ${violations.length} stub redirect(s) — a redirect husk is a fake-green:\n`,
    )
