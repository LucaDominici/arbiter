#!/usr/bin/env node
// python-library — exit code contract gate
...
if (process.argv.includes('--help') || process.argv.includes('-h')) {
  process.stdout.write('Usage: node scripts/check-exit-code-contract.mjs [dir...] [--help]\nScans scripts for exit codes outside the 0=PASS/1=FAIL/2=ERROR contract.\n');
  process.exit(0);
}
...
const PATTERNS = [
  { re: /process\.exit\(\s*(-?\d+)\s*\)/, label: "process.exit(N)" },
];
