...
let allowLocalChecks = process.env.ARBITER_ALLOW_LOCAL_CHECKS === '1'; // #2679: explicit opt-in only
const _rawArgs = process.argv.slice(2);
const _gateUsage =
  'Usage: node scripts/check-all.mjs [L1|L2|L3|L4 | check|gate|full|simulate-nightly|simulate-weekly] [--level <L1|L2|L3|L4>] [--json [path]] [--dry-run] [--gate <name>] [--allow-local-checks]';
...
// gates", never a missed or duplicated execution.
{
  ...
  if (!existsSync(_localSlotPath)) {
    ...
    // so it can never be the trust boundary; require an opt-in the repository
    // cannot set on its own — an env var (CI) or a CLI flag (a human, one run
    // at a time). Fail closed: absent opt-in, print and SKIP, never execute.
    console.log(
      '[CHECK] local checks ... SKIP (scripts/check-all.local.json present but ' +
        'ARBITER_ALLOW_LOCAL_CHECKS is not set — set ARBITER_ALLOW_LOCAL_CHECKS=1 or pass ' +
        '--allow-local-checks to run it)',
    );
    pushResult('local checks', 'SKIP', 0);
  } else {
    let _localChecks;
