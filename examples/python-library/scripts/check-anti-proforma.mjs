#!/usr/bin/env node
// python-library — anti-proforma test gate (INV-118, §R-41)
...
const HELP = [
  'Usage: node scripts/check-anti-proforma.mjs [options]',
  '',
  ...
  '',
  'Options:',
  '  --dir=<path>     Directory to scan for test files (default: cwd)',
...
function main() {
  ...
  for (const file of testFiles) {
    ...
    for (const block of blocks) {
      totalTests++;
      if (block.exemptComment) { exemptTests++; continue; }
...
try {
  ...
  process.stderr.write(`[anti-proforma] ERROR: ${err instanceof Error ? err.message : String(err)}\n`);
  process.exit(2);
}
