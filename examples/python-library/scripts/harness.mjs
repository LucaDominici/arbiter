...
for (const gate of GATES) {
  const scriptPath = join(__dirname, `${gate}.mjs`);
  if (!existsSync(scriptPath)) {
    process.stdout.write(`  harness: ${gate} — skip (script not found)\n`);
    continue;
  }
  executed++;
...
// Vacuous-green floor: a run that executed no gate verified nothing — never
// report a passing exit (anti-fake-green).
if (executed === 0) {
