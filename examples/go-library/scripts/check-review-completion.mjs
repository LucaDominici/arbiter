...
function activeTreatmentFailure(sidecar, task) {
  ...
  try {
    ...
    if (!isRecord(state) || state['taskId'] !== task) return null
    const treatment = state['treatment']
    if (!isRecord(treatment)) return 'active task has no persisted ship treatment'
    ...
    if (
      sidecar.count !== count ||
      JSON.stringify(sidecar.auditors) !== JSON.stringify(verticals) ||
      sidecar.treatmentHash !== hash
    ) {
      return 'review sidecar does not match the active ship treatment'
    }
    return null
    // FAIL-OPEN-INTENT: the returned diagnostic is added to blocking failures by the caller.
  } catch (err) {
    return `cannot validate active ship treatment: ${errorMessage(err)}`
  }
}
...
function main() {
  ...
  if (treatmentFailure) {
    process.stdout.write(`[check-review-completion] FAIL: ${treatmentFailure}\n`)
    return 1
  }
