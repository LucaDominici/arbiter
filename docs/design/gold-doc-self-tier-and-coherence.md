...
| new matcher beside `:66`        | Fixed-local helper form for emitted thin runners: `/runLocalArbiter\(\['([a-z][a-z0-9-]*)'/g` over `src/templates/scripts/*.mjs.ejs` (the four document runners). Plus the fenced-line form `^(?:npx\s+(?:--no-install\s+)?)?arbiter\s+([a-z][a-z0-9-]*)` inside fenced code blocks (the `ship.md:58` shape), sharing the existing `PROSE_STOPWORDS` filter. |
