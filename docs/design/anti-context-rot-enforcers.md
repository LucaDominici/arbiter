...
```json
{
  ...
  "refutations": [{ "target": "<finding id>", "verdict": "UPHELD|REFUTED",
                    "citations": [] }],
...
1. **Bypass-rate ceiling.** Parse `.arbiter/evidence/bypass-log.jsonl` (shape
   ...
   `scripts/data/ceremony-thresholds.json` (default 12/month; per-env overrides; solo default
   20) — data file, not `arbiter.json`, to avoid touching the required-config schema (T0/T2
   brick risk).
...
(`stop-evidence-guard.mjs:134-152`), which nudges to *drain* findings already in the spool;
this detects that *zero were ever captured*.
