...
- **Dogfood:** `arbiter doc-set --plan` on coach-system (peer-review = small): plan lists the small
...
raise for the *other* eight rows remains T1b's job.
...
- **T3:** `arbiter doc-set --plan/--apply` live; ≥1 real skeleton per bound row of §1.2(b);
  banner-upgrade + withheld tests green; coach-system dogfood run recorded.
