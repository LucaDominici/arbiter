...
- Same class, docs-side — **partially resolved (#2017)**: `CONTEXT_PACK_SPEC.md.ejs` /
  ...
  `scripts/emit-context-pack.mjs` does exist in self. **Still open:**
  `.claude/agents/context-checker.md:28` and its emitted twin
  `src/templates/claude/agents/context-checker.md.ejs:28` tell every governed repo that
  `CONTEXT_PACK.md` is "generated by `scripts/emit-context-pack.mjs`" — a script that is never
  emitted to those repos. The emitted-surface half of this cluster stands.
