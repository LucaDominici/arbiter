...
- CI `Full Gate (L2, INV-59)` **failed** on the pinned commit `40062674` (run 29194714679):
  `format` check, file `__tests__/templates/_monthly-render.test.ts` — introduced *by that same
  commit* (PR #1939). Reproduced locally on today's tip: `npx prettier --check` still fails on it.
...
- **Why blocking:** the product's one-line pitch is *"governance that can't be faked"*. Publishing
...
- Clean repo without `node_modules` (i.e., right after `git clone`): init generates
  ...
  just wrote. Result: **"arbiter init aborted"** (exit 1), *after* writing 205 files, mutating
...
- Same class, docs-side: `CONTEXT_PACK_SPEC.md.ejs` / `CONTEXT_SLICE_SPEC.md.ejs` are rendered by
  **no generator** (zero references in `src/**/*.ts`) and document `scripts/emit-context-pack.mjs`,
  which exists neither in self nor in any emitted project — a dead template promising a
  nonexistent tool.
- The guard that should catch this (H7, CLI-surface coherence on the *emitted* agent surface) is
  design-only; its design doc isn't even on main (see B5 note).
- **Why blocking:** these files are *in the npm tarball* and are the operating instructions given
...
- `node scripts/check-doc-set.mjs` on self: **`[tier: solo]`** — 12 applicable docs, 36 N/A.
  ...
  self to the *solo* column. The enterprise-grade governance product audits itself against the
...
- Also unverifiable offline: ownership of the **`@arbiter` npm scope** (`@arbiter/cli` is 404 —
  ...
  typing `npx arbiter`). If the scope isn't yours, nothing else matters.
...
| H6 self passes weak bar | OPEN | OPEN — and now *codified* (self = solo tier, proven live) |
...
emitted *and wired* (verified in the virgin repo, `scripts/check-all.mjs:600–601`) — but the
