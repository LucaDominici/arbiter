...
| M15 | Bypass accounting + ceremony detector                   | footer rationale + append-only log; detector                                                  | **EXISTS** (#1949)                            | `scripts/check-commit-footer-rationale.mjs` (INV-119); `.arbiter/evidence/bypass-log.jsonl`; `scripts/check-bypass-ceremony.mjs` (thresholds `scripts/data/ceremony-thresholds.json`, ledger `scripts/data/advisory-ledger.json`); doctor row `src/commands/doctor/health.ts`; wired advisory `scripts/check-all.mjs`; `__tests__/scripts/check-bypass-ceremony.test.ts` |
...
All six are wired **advisory** (`runWarnCheck`) with dated promoteBy entries in
...
INV. Two rows above stay **PARTIAL** on purpose and are the honest residual: the M3
