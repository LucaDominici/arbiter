...
| M1  | Graph-coverage tier qualification excludes doc-only diff | `gatherTierSignals` drops files graphify never AST-indexes (`.md`, `.arbiter/evidence/tdd/*.json`) from the graph-coverage candidate set before checking completeness, so a plan file or TDD evidence record no longer widens a task to Standard; a missing code file still does. `worktree prepare` prints an advisory hint when `graphify-out/graph.json` is missing or older than HEAD. | **EXISTS** (#2895)                                                       | `src/commands/ship-tier.ts::gatherTierSignals` (`isGraphUnindexed`); `src/commands/worktree.ts::graphFreshnessHint`                                                                                                                                                                                                                                                                                                                                                     |
