...
**Enforcement.** HARD: `scripts/check-ssot-core.mjs` + `scripts/gen-ssot-core.mjs --check`
(INV-108 core-set exhaustiveness), doc-links integrity (INV-55),
`.claude/hooks/pre-edit-ssot-guard.mjs` (unauthorized SSOT edits blocked at edit time),
INV-115 constraint-scan (every free-text prohibition must resolve to a verified enforcer —
the Beyoncé rule _as a gate_). INV-28 (SSOT documents must not contradict) is currently
UNENFORCED: `scripts/check-drift.mjs` is INV-89's generated-content drift detector (hash
comparison against a manifest), not a cross-document contradiction check, and no such
check exists anywhere in this repo (#2563; catalog corrected to say so rather than cite a
gate that isn't real).
...
- **Tested (red-path):** a test that _failed before_ the fix and passes after —
  ...
  100% flip coverage of emitted gates, playbook §T3). CANON-25 makes this a
...
PARTIAL: flip coverage is not yet 100% (playbook §T3). CANON-25 (#2301) closed the
...
| M5  | SSOT-first                                              | core-set + links + edit guard + constraint-scan (INV-28 contradiction check: UNENFORCED, #2563) | **PARTIAL**                                   | `scripts/gen-ssot-core.mjs`/`check-ssot-core.mjs` (INV-108); `scripts/check-doc-links.mjs` (INV-55); `.claude/hooks/pre-edit-ssot-guard.mjs`; `scripts/check-constraint-scan.mjs` (INV-115)                                                                                                                                                                                                                                                                             |
...
| M11 | Flip-coverage 100% of emitted gates                     | extend flip harness                                                                             | **PARTIAL** (absence family closed, #2301)    | `scripts/check-guard-flip.mjs`; `scripts/lib/gate-roster.mjs` (family derived from `check-all.mjs`); `scripts/data/inversion-proof-registry.json` (banked residue, 16 rows); CANON-25; playbook §T3                                                                                                                                                                                                                                                                     |
