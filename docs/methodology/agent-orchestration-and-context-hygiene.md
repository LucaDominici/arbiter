...
| M4  | Finding-loss detector                                   | Stop hook, advisory default / hard via env                                            | **EXISTS** (implement-but-not-activated, #1948)   | `.claude/hooks/stop-finding-loss.mjs`; `__tests__/hooks/empirical/stop-finding-loss.test.ts`; `.arbiter/hooks-manifest.json`; design doc §E6b. Not wired into the Stop chain (OD-14, owner decision).                                                                                                                 |
