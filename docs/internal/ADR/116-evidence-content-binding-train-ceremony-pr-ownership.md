...
| Pre-code review follows collaboration mode             | `trunk-solo` does not dispatch a red-team panel before TDD or require an evidence file for one; the root implementer owns the plan and threat model, then one independent final diff review remains mandatory. `peer-review` and `gated-review` retain tier-N pre-code red-team evidence. No new override flag or risk axis is introduced. |
...
- `trunk-solo` spends model judgment after executable work exists: one final diff reviewer can
  inspect code and tests. Plan completeness, TDD, acceptance fit, the full gate and CI remain the
  safeguards before merge; modes with independent human roles keep the pre-code panel.
