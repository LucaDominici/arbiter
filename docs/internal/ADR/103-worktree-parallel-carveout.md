...
| 2. Distinct branch    | **Git's** property, not arbiter's: `git worktree add -b` fails if the branch exists, and git refuses to check out one branch in two worktrees. `branchNameFor(taskId, slug)` (`src/worktree/paths.ts:52`) derives the branch from the task id and optional slug, so two opens of the same task _and_ slug collide — the guarantee rests on git refusing an existing ref, not on the naming. Reachable only via the `runWorktreeOpen` path — a harness-created worktree bypasses it. | Hard, but only on the sanctioned path |
...
grouping, sequencing — lives in the model, inside the `wave-drain` skill. The harness is what
spawns agents (CANON-12). (At the time of the decision the un-wired `defaultIssueRunner` seam was
the concrete expression of that boundary; it was pruned as dead code in #1921 and no longer exists.)
...
D1-D4 below are rule-50's; D5 is the `gate-exec.ts` docstring and D6 the 0.5.0 changelog — same
treatment, different surviving source.
rule-50 has been corrected, and the disagreements are recorded here so the correction is auditable.

| #   | What the surviving record said                                                                                                | What the code does                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| --- | ----------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
...
| D5  | The original `gate-exec` docstring said release was "guaranteed on the flock holder's fd death — including SIGKILL/OOM-kill". | Before #2346 this was true only of the `flock` process's descriptor, not of the whole gate. Since #2346, a detached supervisor acquires the lock on an fd shared with Node and closes it in the payload subshell: killing Node alone leaves the supervisor holding it, while killing the supervisor makes Node tear down its process group and, on Linux, descendants holding an inherited sentinel fd before releasing its copy. The deliberate-close residual is recorded below.                                                                                                                                                                                             |
...
- **The `-o` trade-off (#2196), resolved by #2346.** The E2E campaign (2026-08-03) found that the
  lock fd was inherited by every descendant, so a gate that backgrounded a daemon held the
  repo-wide mutex after exiting. #2196 closed that leak, but killing its sole long-lived `flock`
  holder then released the mutex while the gated command still ran. #2346 replaced that topology:
  a short `flock 3` acquisition leaves the locked open-file description in both Node and a detached
  supervisor, while the payload subshell closes fd 3 before `exec`. Neither holder can die alone
  and expose an ordinary running payload; supervisor death triggers process-group teardown plus a
  Linux procfs sweep for descendants that inherited a separate sentinel fd before Node releases
  its copy.
- **Escaped-descendant cleanup has a deliberate-close ceiling.** Ordinary `setsid`, re-parenting,
  and double-forking retain the sentinel and are covered. A determined payload can close the
  sentinel fd before leaving the supervisor process group (as a daemon that closes all inherited
  fds commonly does); it then has neither ancestry nor an inherited handle that `gate-exec` can
  identify. If the supervisor subsequently dies, Node can release the mutex while that payload is
  still running. Systems without readable Linux `/proc` have the broader, explicit group-only
  limit and emit a runtime warning. Closing this last window requires OS containment outside the
  current process model (for example a cgroup/systemd scope), not restoring payload inheritance of
  the lock fd removed by #2196.
...
- **Killing `arbiter` does not kill the gate** (D5). The orphaned supervisor deliberately keeps
  the mutex while its gated command runs. `arbiter doctor`'s `checkGateMutex` (#2196/#2346) reports
  the lock path and supervisor/holder/waiter count, but **repair is not** automatic:
  `doctor recover-lock` handles only `.arbiter/.lock` and `.arbiter/kit.lock`, so immediate release
  still requires manually killing the supervisor process group.
