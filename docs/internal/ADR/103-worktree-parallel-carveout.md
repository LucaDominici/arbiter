...
- **Killing `arbiter` does not kill the gate** (D5). `runGateExec` spawns `flock` as a child and
  installs no process-group teardown, so the orphaned `flock` keeps the lock. Diagnosis is covered
  — `arbiter doctor`'s `checkGateMutex` (#2196) reports the lock path and the holder/waiter count —
  but **repair is not**: `doctor recover-lock` handles only `.arbiter/.lock` and `.arbiter/kit.lock`,
  so releasing a leaked gate mutex is still a manual kill of the holding process group.
