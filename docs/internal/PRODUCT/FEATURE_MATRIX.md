...
| REQ-016 | Module boundary enforcement | N77 | L2 | Partial | src/generators/boundaries.ts |  |  | #2244 | ESLint-import-boundaries (TS); cross-stack not fully covered. OPEN (#2244): no citable functional-tier proof — the `fsd boundaries` gate runs only in greenfield-first-run.test.ts's frontend-spa cell (RED on this tree, CI-unwired) and is gateFilePresent-conditional, so exit-0 is SKIP-compatible; the hexagonal scripts/check-boundaries.mjs is never emitted (architectureStyle='none'). Unit-tier coverage at __tests__/generators/boundaries.test.ts + the per-stack go/python/rust boundaries tests |  | E2E |
...
| REQ-036 | Behavioral testing (BDD / Cucumber) |  | L3 | Partial | src/generators/behavioral-tests.ts |  |  | #2244 | BDD scenario generation; @ignore tag hard-fail (INV-40). OPEN (#2244): partial functional-tier proof only — fixture-functional.test.ts proves the generated BDD suites do not break the build (no unresolved godog/cucumber import), but the INV-40 @ignore HARD-FAIL is never seeded at any tier, so the blocking behaviour this row claims is unproven; the `BDD @ignore check` gate runs only in the L2 lane with no per-gate assertion. Unit-tier coverage at __tests__/templates/behavioral-tests-render.test.ts |  | E2E |
| REQ-037 | Contract testing (Pact) |  | L3 | Partial | src/generators/contract-testing.ts |  |  | #2244 | Pact broker + schema registry gates (INV-41/42). OPEN (#2244): no functional-tier proof — every contract-pact gate needs contractType rest-owned/rest-public AND debt gates on; the only public-API functional fixture inits at L1 (debt gates off), and even where emitted the gate's condition is `process.env.PACT_BROKER_BASE_URL`, so it prints SKIP. The generic `contract tests` that does run at L2 is not Pact and not INV-41/42. Unit-tier coverage at __tests__/generators/contract-testing-render.test.ts |  | E2E |
| REQ-038 | Evidence retention & audit bundle |  | L4 | Partial | src/generators/evidence-retention.ts |  |  | #2244 | INV-90; evidence-bundle gate + retention policy. OPEN (#2244): no functional-tier proof — the `evidence gate (INV-33)` is L3-only and enableEvidenceHarness is L4-only, while no functional cell exceeds L2, so this generator's artifacts (evidence-rotate.mjs, evidence-prune.mjs, done-evidence.mjs, evidence-files.json) are never emitted nor executed at this tier. Unit-tier coverage at __tests__/scripts/check-evidence-bundle.test.ts + __tests__/generators/evidence-retention.test.ts |  | E2E |
...
| REQ-046 | Local CI wrapper |  | L2 | Partial | src/generators/local-wrapper.ts |  |  | #2244 | act-based local CI; parity gate. OPEN (#2244): functional-tier coverage is vacuous — `local-ci-parity (INV-59)` IS emitted and executed in the two L2 trunk-solo cells, but the check short-circuits with `[skip] static parity: .github/workflows absent` (no fixture ships workflows) and its runtime half needs `gh` plus a CI artifact, so exit 0 proves nothing; the emitted Makefile/run.sh are never executed by any test. Unit-tier coverage at __tests__/generators/local-wrapper.test.ts + __tests__/scripts/check-local-ci-parity.test.ts |  | E2E |
