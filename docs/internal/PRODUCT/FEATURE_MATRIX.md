...
| REQ-043 | Changeset / release tooling |  | L1 | Partial | src/commands/task.ts | __tests__/commands/task.test.ts,__tests__/scripts/changeset-version.test.ts |  | #2246 | arbiter lifecycle lifecycle + changeset integration |  | GATE |
...
| REQ-053 | /ship orchestration synthesis (single narrative, #1216) |  | L1 | Verified | src/commands/task-ship.ts | __tests__/templates/commands-claude.test.ts | .claude/commands/ship.md | #1216 | /ship is the sole orchestration entrypoint; arbiter lifecycle is the low-level engine/CLI; dual-sided dogfood parity; FEATURE_MATRIX + ADR-088 |  | GATE |
