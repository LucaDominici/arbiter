...
| ---------- | ---------------------------------------------------------------- | ------------------------------- | ----- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------- | --------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- | ------------------ |
| REQ-001 | Architecture enforcement (hexagonal / layered) | N01,N02,N03,N04,N05,N06,N07 | L2 | Partial | src/generators/boundaries.ts |  |  | #2244 | ArchUnit (Java) + ESLint boundaries (TS); cross-stack coverage partial. OPEN (#2244, prereq #2257): no functional-tier proof — every functional fixture inits with architectureStyle='none', so the `boundaries` and java `architecture tests` gates are never EMITTED, let alone executed; ArchUnit never runs at this tier. Unit-tier coverage exists at __tests__/generators/archunit.test.ts + __tests__/generators/boundaries.test.ts. Closing needs a hexagonal fixture cell, not just a new assertion |  | E2E |
...
| REQ-005 | Test profiles & TDD evidence | N24,N25,N26 | L2 | Partial | src/generators/evidence-retention.ts |  |  | #2244 | Coverage thresholds + TDD evidence gate wired at L2+. OPEN (#2244, prereq #2257): no functional-tier proof — coverage thresholds are never emitted (coverageEnabled requires LOC>=1000, false for every fixture), and the `tdd-evidence (INV-131)` gate is emitted by check-all.ts, not by this row's code_ref. Unit-tier coverage at __tests__/generators/evidence-retention.test.ts + __tests__/generators/coverage.test.ts; evidence-WRITING is proven at the functional tier but by task-command code (packaged-artifact.test.ts asserts the .arbiter/evidence/tdd/#NNN.json schema) |  | E2E |
| REQ-006 | Integration / mutation / behavioral / contract testing | N27,N28,N29,N30,N31,N32,N33 | L2 | Partial | src/generators/integration-testing.ts |  |  | #2244 | Mutation (Java L2+), BDD, contract testing all behind L2+ gates. OPEN (#2244, prereq #2257): no citable functional-tier proof — the `integration tests`/`contract tests`/`behavioral tests` gates only run in the L2 lane, whose only two harnesses (greenfield-first-run, virgin-init-matrix) are both RED on this tree, and even there exit-0 is SKIP-compatible (no per-gate assertion). Mutation is never emitted (L3+; no functional cell exceeds L2). Unit-tier coverage at __tests__/generators/integration-testing.test.ts + __tests__/generators/mutation.test.ts |  | E2E |
| REQ-007 | CI/CD pipeline & workflow runners | N34,N35,N36,N37,N38,N39,N40 | L2 | Partial | src/generators/github.ts |  |  | #2244 | GH Actions workflows generated; workflow-runner gate wired. OPEN (#2244, prereq #2257): no functional-tier proof — useGitHub is false in every functional fixture (no --github, ARBITER_GITHUB unset), so ci-tiers/action-pins/workflow-perms/workflow-runners/sha-pinning/job-naming are never emitted, and no fixture ships .github/workflows, so the inline `ci alignment` check ENOENT-returns an empty gate set and passes vacuously. Unit-tier coverage at __tests__/generators/github.test.ts + __tests__/scripts/check-ci-tiers.test.ts; emission-only proof at virgin-init-matrix.test.ts |  | E2E |
| REQ-008 | E2E & performance testing | N41,N42,N43,N44 | L2 | Partial | src/generators/playwright-ts.ts |  |  | #2244 | Playwright (TS/Python) + k6 perf; archetype-gated. OPEN (#2244, prereq #2257): no functional-tier proof — the `playwright e2e` gate requires L2 AND a frontend-spa/backend-web-db archetype, and every cell that reaches the L2 lane is a `library` archetype, so it is never emitted; k6 has no gate in the registry at all, so the perf half is ungated everywhere. Unit-tier coverage at __tests__/generators/playwright-ts.test.ts + __tests__/generators/perf-k6.test.ts |  | E2E |
| REQ-009 | Gate script validation | N45,N46,N47,N48,N49,N50 | L2 | Partial | src/generators/check-all.ts | __tests__/integration/e2e/functional/fixture-functional.test.ts |  | #2244 | check-all.mjs generated + wired; 29 gates wired-but-untested (GAP). RTM #2244 E2E evidence: fixture-functional.test.ts spawns the generated `scripts/check-all.mjs L1` in 8 real-project fixtures and asserts it actually executes (status != 127, no `Cannot find module`, no unresolved godog/cucumber import) and exits 0 — the strongest functional-tier row in the matrix |  | E2E |
| REQ-010 | Quality gate scripts | N51,N52,N53,N54 | L2 | Partial | src/generators/debt-gates.ts |  |  | #2244 | Debt ratchet, duplication, bloat, dead-code gates generated. OPEN (#2244, prereq #2257): no citable functional-tier proof — the one seeded RED that proves the knip `dead code` gate blocks lives in greenfield-first-run.test.ts, which is RED on this tree AND executed by no CI workflow; `duplication`/`circular deps`/`debt ratchet` are L2-lane and SKIP-compatible; `bloat` has no gate in the registry at all. Unit-tier coverage at __tests__/generators/debt-gates.test.ts |  | E2E |
| REQ-011 | Security toolchain | N55,N56,N57,N58 | L2 | Partial | src/generators/security.ts |  |  | #2244 | Gitleaks, npm audit, STRIDE enforcement; SAST level-gated. OPEN (#2244, prereq #2257): no citable functional-tier proof — the binary-guarded gitleaks execution against this generator's .gitleaks.toml happens only in virgin-init-matrix.test.ts, which is RED on this tree; `PII scan` runs in every L1 cell but is SKIP-compatible with exit 0; and the one A-grade seeded secret-scan RED belongs to anti-drift-validators.ts, NOT to this row's code_ref, so it must not be borrowed. Unit-tier coverage at __tests__/generators/security.test.ts |  | E2E |
...
| REQ-016 | Module boundary enforcement | N77 | L2 | Partial | src/generators/boundaries.ts |  |  | #2244 | ESLint-import-boundaries (TS); cross-stack not fully covered. OPEN (#2244, prereq #2257): no citable functional-tier proof — the `fsd boundaries` gate runs only in greenfield-first-run.test.ts's frontend-spa cell (RED on this tree, CI-unwired) and is gateFilePresent-conditional, so exit-0 is SKIP-compatible; the hexagonal scripts/check-boundaries.mjs is never emitted (architectureStyle='none'). Unit-tier coverage at __tests__/generators/boundaries.test.ts + the per-stack go/python/rust boundaries tests |  | E2E |
...
| REQ-030 | ADR SSOT gate |  | L2 | Partial | scripts/check-adr-index.mjs | __tests__/scripts/check-adr-index.test.ts |  | #2246 | INV-107; 11 ADRs extracted (077–087) |  | GATE |
...
| REQ-032 | Frontend governance (FE constitution) |  | L2 | Partial | src/generators/frontend-governance.ts |  |  | #2244 | INV-102–106; DTCG tokens, Vitest browser, WCAG 2.2. OPEN (#2244, prereq #2257): no functional-tier proof — every FE gate (fe boundaries INV-102/103/104, token discipline INV-105, i18n literals/parity INV-106, fe coverage ratchet, bundle size budget) requires L2 AND a frontend archetype, and the tier's only FE cell (greenfield ts/frontend-spa) inits at L1, so none of INV-102..106 is emitted. No DTCG/Vitest-browser/WCAG assertion exists anywhere at this tier. Unit-tier coverage at __tests__/generators/frontend-governance.test.ts |  | E2E |
...
| REQ-036 | Behavioral testing (BDD / Cucumber) |  | L3 | Partial | src/generators/behavioral-tests.ts |  |  | #2244 | BDD scenario generation; @ignore tag hard-fail (INV-40). OPEN (#2244, prereq #2257): partial functional-tier proof only — fixture-functional.test.ts proves the generated BDD suites do not break the build (no unresolved godog/cucumber import), but the INV-40 @ignore HARD-FAIL is never seeded at any tier, so the blocking behaviour this row claims is unproven; the `BDD @ignore check` gate runs only in the L2 lane with no per-gate assertion. Unit-tier coverage at __tests__/templates/behavioral-tests-render.test.ts |  | E2E |
| REQ-037 | Contract testing (Pact) |  | L3 | Partial | src/generators/contract-testing.ts |  |  | #2244 | Pact broker + schema registry gates (INV-41/42). OPEN (#2244, prereq #2257): no functional-tier proof — every contract-pact gate needs contractType rest-owned/rest-public AND debt gates on; the only public-API functional fixture inits at L1 (debt gates off), and even where emitted the gate's condition is `process.env.PACT_BROKER_BASE_URL`, so it prints SKIP. The generic `contract tests` that does run at L2 is not Pact and not INV-41/42. Unit-tier coverage at __tests__/generators/contract-testing-render.test.ts |  | E2E |
| REQ-038 | Evidence retention & audit bundle |  | L4 | Partial | src/generators/evidence-retention.ts |  |  | #2244 | INV-90; evidence-bundle gate + retention policy. OPEN (#2244, prereq #2257): no functional-tier proof — the `evidence gate (INV-33)` is L3-only and enableEvidenceHarness is L4-only, while no functional cell exceeds L2, so this generator's artifacts (evidence-rotate.mjs, evidence-prune.mjs, done-evidence.mjs, evidence-files.json) are never emitted nor executed at this tier. Unit-tier coverage at __tests__/scripts/check-evidence-bundle.test.ts + __tests__/generators/evidence-retention.test.ts |  | E2E |
...
| REQ-044 | Plugin system |  | L2 | Partial | src/commands/plugin.ts | __tests__/scripts/check-plugin-api-stability.test.ts |  | #2246 | Plugin API stability gate (INV-55). FINDING (#2246 backfill): code_ref src/commands/plugin.ts does not exist — the real modules are src/integrations/plugin-schema.ts, src/utils/plugin-loader.ts, src/utils/plugin-worker.ts; repointing code_ref is out of this citation-only issue's scope, tracked as a note |  | GATE |
...
| REQ-046 | Local CI wrapper |  | L2 | Partial | src/generators/local-wrapper.ts |  |  | #2244 | act-based local CI; parity gate. OPEN (#2244, prereq #2257): functional-tier coverage is vacuous — `local-ci-parity (INV-59)` IS emitted and executed in the two L2 trunk-solo cells, but the check short-circuits with `[skip] static parity: .github/workflows absent` (no fixture ships workflows) and its runtime half needs `gh` plus a CI artifact, so exit 0 proves nothing; the emitted Makefile/run.sh are never executed by any test. Unit-tier coverage at __tests__/generators/local-wrapper.test.ts + __tests__/scripts/check-local-ci-parity.test.ts |  | E2E |
