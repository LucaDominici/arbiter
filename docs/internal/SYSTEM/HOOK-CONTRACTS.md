...
| `skill-forced-eval.mjs`         | Stop               | \*           | read, stderr-block, write    | `.claude/.task/status.json`, transcript, receipt at `HEAD` (git), session marker                                                                                                                                       | SAFE (#2383/#2861; exit 2 on edit without phase Skill(tdd) or green/refactor receipt)                                  |
| `guard-task-completion.mjs`     | Stop               | \*           | read, write                  | `.claude/.task-*`, `.claude/.task/status.json` (treatment), `.arbiter/agents-dispatched.json`, `.arbiter/evidence/tdd/`, assistant response, session marker at `.claude/.task/guard-task-completion-<session_id>.json` | SAFE (#2885; negation/quoted-span aware, blocks below the persisted reviewer floor, session-bound re-entry marker)     |
...
| `stop-evidence-guard.mjs`       | Stop               | \*           | read (transcript, git)       | `.arbiter/evidence/*`, `.claude/.task/`                                                                                                                                                                                | SAFE (#2885: shares the negation/quoted-span classifier so it does not re-block a message the completion guard allows) |
