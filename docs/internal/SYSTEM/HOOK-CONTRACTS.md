...
| `guard-task-completion.mjs`     | Stop               | \*           | read, write                  | `.claude/.task-*`, `.claude/.task/status.json` (treatment), `.arbiter/agents-dispatched/<task>.json`, `.arbiter/evidence/tdd/`, assistant response, session marker at `.claude/.task/guard-task-completion-<session_id>.json` | SAFE (#2885; blocks below the persisted reviewer floor, session-bound re-entry marker)            |
