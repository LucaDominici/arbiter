...
**`hook` may be per-track (#2554).** A plain string is the shorthand for "the same hook on every
track `track` names". A `both` row whose hook only exists on one side declares it instead as
`{ "self": "<path or n/a>", "target": "<path or n/a>" }` — each leg independently a path or `n/a`,
so a gate that ships on both tracks can still say its hook covers only self.
...
```json
{
  ...
  "schemes": [
    ...
    },
    {
      ...
      "hook": "n/a",
      "status": "active",
      "note": "NOT the bare `A` prefix the programme plan proposed: `A2`, `A4` and `A11` are already in use as audit-wave and action-plan ids (docs/internal/DEVELOPMENT/REAL-PROJECT-TESTING.md, docs/internal/ADR/037-evidence-harness-target-projects.md, .claude/hooks/closer-mode-guard.mjs), so `A` would have collided on the day it was registered — this registry catching its own plan is the mechanism working. A fixed enumeration rather than minted ids, like CANON: the twelve slots are arc42's, and a project neither invents nor retires one. The pattern is deliberately the loose `[0-9]{2}` shared by every other scheme rather than an exact 01-12 alternation: this registry's job is collision detection, which needs a pattern the gate can expand into a sample, and check-arc42-slots.mjs is what enumerates the real twelve. `hook: n/a` — the artifact is prose under a heading, not a schema'd document, so there is nothing for post-edit-artifact-schema.mjs to validate at edit time; the gate is the enforcement."
    },
    {
      "prefix": "MS",
      "pattern": "^MS-[0-9]{2}$",
      ...
      "track": "both",
      "tool": "arbiter graph build",
      "hook": { "self": ".claude/hooks/post-edit-artifact-schema.mjs", "target": "n/a" },
      "status": "active",
      "note": "Wave 3. Claims the prefix now so no NEW milestone is numbered into the methodology namespace; the SSOT file, the gate and the MILESTONE node land together, and the 33 historical MN headings in MILESTONES.md are superseded rather than renamed in place. The `graphNode` column is NOT set: wave 8 taught the meta-gate to verify that a declared NodeKind exists in src/graph/model.ts and found that six rows named one the graph has never had — the same shape as the hook column, a statement of intent reading as a statement of fact. The node kind is still owed; what carries this scheme's data to a consumer TODAY is the gate's own projection, the mechanism the milestone wave established, so the claim is withdrawn rather than left standing until the builder exists. Wave 8 landed the Track-B emission that this row was staged for: scripts/check-milestones.mjs.ejs plus schemas/milestone.schema.json, emitted together and wired at L1 in the target roster. The emitted twin reads a fenced JSON block in docs/MILESTONES.md rather than YAML — `yaml` is arbiter's dependency, no emitted script imports it, and a Go or Python project receives 77 .mjs gates and NO package.json in which to declare one — which is the same constraint that already shaped docs/SOURCES.md and docs/USE_CASES.md. Rules identical; reader, path and INV namespace (AGENTS.md, not the TypeScript catalog) are the three pinned divergences. #2554: `hook` is now per-track — post-edit-artifact-schema.mjs DOES cover docs/internal/PRODUCT/MILESTONES.yml on arbiter's own track (wave 8 added the yaml extract mode for exactly that, and the meta-gate's coverage check verifies it), so `self` names it; `target` stays `n/a` because no emitted hook twin exists — a real limitation of the mechanism, not of the column, now that the column can say so directly."
    },
    {
      "prefix": "SRC",
      "pattern": "^SRC-[0-9]{3}$",
