...
# 3. Re-derive the snapshot envelope so checksum + $schemaVersion match.
arbiter doctor --repair-state

# 4. Install the stable channel.
npm i @arbiter/cli@latest

# 5. Verify the install works against the restored config.
arbiter --version
```

If the snapshot backups (`.arbiter-generated.json.bak.<ts>`) are still present, step 3 can be skipped — just copy the most recent backup over `.arbiter-generated.json` and `arbiter` will verify the checksum on load.

The `arbiter --channel <name>` self-update flag (#662) will package this into one command; until then, the steps above are the canonical procedure.
