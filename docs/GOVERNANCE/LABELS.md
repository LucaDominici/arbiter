...
| Label                                     | Why removed                                                                                           |
| ----------------------------------------- | ----------------------------------------------------------------------------------------------------- |
| `wave-0` / `wave-1` / `wave-2` / `wave-3` | ephemeral wave tags; arbiter's wave engine computes waves internally and never applies them           |
| `quality-wave`                            | wave-tracking marker with no consumer                                                                 |
| `approved`                                | duplicates GitHub's native PR-review "approved" state; distinct from the consumed `approved-by-human` |
| `size: L` / `size: XL`                    | `taskTiers` enum is `{XS,S,Standard}` only — never produced                                           |
