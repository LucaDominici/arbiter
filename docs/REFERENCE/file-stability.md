...
### Derived files update in their own transaction

`configure` writes only `arbiter.json`. Generator-owned files such as
`.claude/commands/drain.md` are refreshed by the explicit `arbiter update` step. This keeps
the configuration write atomic: a failed or preserve-withheld generated-file update cannot
leave `configure` half-applied. The generator still derives `/drain`'s worktree cap from the
stored configuration, and `update` reports any preserved output it cannot replace.
