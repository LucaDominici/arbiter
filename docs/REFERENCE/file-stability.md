...
A full-registry `arbiter update` now partitions the manifest keys it did not visit this run:

| bucket    | condition                                                | action                               |
| --------- | -------------------------------------------------------- | ------------------------------------ |
| `retire`  | `.claude/hooks/*.mjs` **and** `sha256(disk) == baseline` | **deleted**, named in the run report |
| `orphans` | `.claude/hooks/*.mjs`, disk differs from the baseline    | kept, reported for manual removal    |
...
lossless operation, so it is reported, not warned about, and the counts ride in the `--json` summary
(`retired`, `orphaned`).
