...
| `skill-forced-eval.mjs`      | —       | LOW      | `arbiter lifecycle advance` phase gates | No final-response interception         |
| `post-edit-dispatch.mjs`     | —       | LOW      | None                                    | No equivalent                          |
| `guard-task-completion.mjs`  | —       | MEDIUM   | `arbiter lifecycle advance` phase gates | No final-response interception         |
| `guard-done-evidence.mjs`    | —       | MEDIUM   | `arbiter lifecycle advance` phase gates | No final-response interception         |
...
- Codex completion/TDD coverage is delayed to native `arbiter lifecycle advance` phase gates; Codex does
