...
> **Command:** `arbiter check evidence` (alias `arbiter check evidence`)
...
`arbiter check evidence [--dir <dir>]` resolves the SUMMARY.json to verify in this
order:
...
`arbiter check --dir <dir> evidence` and `arbiter check evidence --dir <dir>` verify
the same `<dir>`. Relative values resolve from the current directory, absolute values are
used as-is, and the current directory is used only when `--dir` is absent (#2683, #2686).
...
shape. The canonical shape written by `<project>/scripts/evidence-collect.mjs` (generated by
arbiter) is:
