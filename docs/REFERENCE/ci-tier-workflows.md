...
| `_label-sync.yml`          | GitHub enabled                             | Bot: sync `.github/labels.yml` → repo labels on push to main (no `workflow_call`)                                                                                                                                                                      |
| `_label-on-approve.yml`    | GitHub enabled                             | Bot: label management on PR review approval                                                                                                                                                                                                            |
| `_ai-draft-check.yml`      | GitHub enabled                             | Bot: AI-authored PR detection (commit trailer, `ai-authored` label, or Bot account, #2552) / labelling (INV-91; `dependabot[bot]` exempt)                                                                                                              |
| `_pr-staleness.yml`        | GitHub enabled                             | Bot: stale-PR sweep                                                                                                                                                                                                                                    |
| `generator-matrix.yml`     | arbiter self only (hand-authored, #1840)   | Generator Matrix: the five DEEP generator E2E cells (TS packaged artifact; Python/Go/Rust/Java fixture-functional), each with only its own toolchain, plus `examples/` drift for ts/python/go — weekly Sun 05:00 UTC, `release: prereleased`, dispatch |
