...
- **N1 — fail-closed disarm**: `scripts/gold-audit.mjs --check --require-baseline` HARD-FAILS when
  ...
  erase the regression record. (Gate wiring of `--require-baseline` into the committed
  `check-all.mjs` gold-audit step is owned by the gate-orchestrator surface; the flag and its
  refusal are shipped + self-tested here.)
