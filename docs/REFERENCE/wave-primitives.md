...
- **Mechanics:** a detached supervisor acquires a Node-opened fd with `flock 3`; both processes
  retain that locked open-file description while the wrapped command runs in a subshell that closes
  fd 3 before `exec`, so backgrounded descendants cannot retain the mutex. If the supervisor dies
  by signal, Node tears down its process group before releasing the final fd copy.
