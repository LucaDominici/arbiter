...
**Declared UNVERIFIABLE**: none in §1–§2 after live-running the CLI to settle the public/
experimental split. Gate-ladder and `.bloat-baseline.json` figures are point-in-time by
construction — read them from their SSOT at whatever moment matters, never from this table. The
GAMP tier split (13/22/20, §1) is independently re-counted from the FEATURE_MATRIX table body
this session (`grep -c` on the tier column), not merely trusted from its own hand-maintained
Distribution table.
