...
| **[adr-index.md](adr-index.md)** | All ADRs (current count: `docs/internal/ADR/`) catalogued one line each, grouped by theme, **plus a gap register** of load-bearing decisions that have no ADR (e.g. the "no new TS engine" B-prune).                               | You want to find or audit a decision.                                  |
