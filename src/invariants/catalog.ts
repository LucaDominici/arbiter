...
export const INVARIANT_CATALOG: readonly Invariant[] = [
  ...
  },
  {
    ...
    description:
      ...
      'a .githook or a workflow, every command in .claude/settings.json, and every node scripts/*.mjs ' +
      'a .claude/commands/*.md playbook instructs an agent to run must resolve to an emitted file. An ' +
      'UNGUARDED-missing reference is a crash-class ghost and ALWAYS fails. A GUARDED-missing ' +
      ...
      'a suppression). Command-doc refs cannot existsSync-guard, so a config-gated one (e.g. the ' +
      'L4-only done-evidence.mjs) must be declared optional. Workflows must additionally SHA-pin every ' +
      'uses: ref (local ./ and docker:// excepted) and name every top-level job. Caught the fleet-wide ' +
      'ci-classify-changes / exitplanmode-banner / build-kit ghosts (#1331) and the ship.md ' +
      'route-auditors.mjs dangling playbook instruction (#1346).',
    enforcement:
