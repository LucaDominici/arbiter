...
export function generateClaude(config: ProjectConfig): ClaudeGeneratorResult {
  ...
  if (existsSync(settingsPath)) {
    ...
    const incoming = JSON.parse(renderTemplate('claude/settings.json.ejs', data)) as Record<string, unknown>;
  ...
  for (const hookFile of staticHooks) {
    results.push(writeFile(join(hooksDir, hookFile), renderTemplate(`claude/hooks/${hookFile}`, data), { skipIfExists: true }));
  }

  results.push(writeFile(join(hooksDir, 'lib.sh'), renderTemplate('claude/hooks/lib.sh.ejs', data), { skipIfExists: true }));
  results.push(writeFile(join(hooksDir, 'post-commit-check.sh'), renderTemplate('claude/hooks/post-commit-check.sh.ejs', data), { skipIfExists: true }));

  // Language-specific hook scripts
  for (const hook of config.languageHooks) {
    if (hook.name !== 'check-no-orphan-todo.sh') {
      const body = hook.body.startsWith('#!/') ? hook.body : `#!/usr/bin/env bash\n${hook.body}`;
      results.push(writeFile(join(hooksDir, hook.name), body, { skipIfExists: true }));
    }
  }
  ...
  for (const cmd of commands) {
    results.push(writeFile(join(commandsDir, cmd), renderTemplate(`claude/commands/${cmd}`, data), { skipIfExists: true }));
  }
