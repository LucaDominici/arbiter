...
const ITALIAN_STOPWORDS_RE =
  /\b(che|non|anche|per|con|come|nel|della|degli)\b/i;
...
export const uiLanguageRule: VerifyPlanRule = {
  ...
  applicability(plan: PlanJsonV1): boolean {
    return plan.files.some((f) =>
      isUiApplicable(
        f.path,
        f.changes?.adds_ui_strings,
        f.changes?.ui_strings?.length ?? 0,
      ),
    );
  },
  evaluate(plan: PlanJsonV1): RuleViolation[] {
    ...
    for (const f of plan.files) {
      ...
      if (strings.length > 0) {
        ...
        f.changes?.adds_ui_strings === true &&
        plan.invariants?.ui_language !== "EN"
      ) {
        violations.push({
          rule_id: "VB-INV-EN-UI",
          severity: "WARN",
